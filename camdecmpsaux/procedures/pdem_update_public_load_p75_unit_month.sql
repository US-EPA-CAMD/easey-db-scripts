create or replace procedure camdecmpsaux.PDEM_Update_Public_Load_P75_Unit_Month 
(
...
begin
    ...
    select  ( oyr.Archive_Ind = 1 )::boolean as Archived,
            prd.Calendar_Year,
            ( 3 * ( prd.Quarter - 1 ) + 1 ) as Begin_Month,
            ( 3 * ( prd.Quarter - 1 ) + 3 ) as End_Month,
            prd.Begin_Date,
            prd.End_Date,
            sbs.User_Id
      into  vArchived,
    ...
    if ( not vArchived ) then
        ...
        delete
          from  camddmw.MONTH_UNIT_DATA
         where  Op_Year = vYear
           and  Op_Month between vBeginMonth and vEndMonth
           and  Unit_Id = any( vUnitIds );
        
    
        insert
          into  camddmw.MONTH_UNIT_DATA 
                (
                    Unit_Id,
                    Op_Year,
                    Op_Month,
        ...
        select  dat.Unit_Id,
                dat.Op_Year,
                dat.Op_Month,
                to_char( to_timestamp ( dat.Op_Month::text, 'MM' ), 'Month' ) as Month_Description,
                ...
                case
                    when sum( dat.So2_Mass_Lbs ) is null then null
                    else round( sum( dat.So2_Mass_Lbs ) / 2000, 3)
                end as So2_Mass,
                ...
                case
                    when sum( dat.Nox_Mass_Lbs ) is null then null
                    else round( sum( dat.Nox_Mass_Lbs ) / 2000, 3)
                end as Nox_Mass,
                ...
                dat.Rpt_Period_Id,
                case
                    when dat.Op_Month in ( 1, 2, 3 ) then 1
                    when dat.Op_Month in ( 4, 5, 6 ) then 2
                    when dat.Op_Month in ( 7, 8, 9 ) then 3
                    when dat.Op_Month in ( 10, 11, 12 ) then 4
                    else null
                end as Op_Quarter,
        ...
        delete
          from  camddmw_arch.MONTH_UNIT_DATA_A
         where  Op_Year = vYear
           and  Op_Month between vBeginMonth and vEndMonth
           and  Unit_Id = any( vUnitIds );
        ...
        insert
          into  camddmw_arch.MONTH_UNIT_DATA_A
                (
                    Unit_Id,
                    Op_Year,
                    Op_Month,
        ...
        select  dat.Unit_Id,
                ...
                vUserid as Userid,
                current_timestamp as Add_Date
          from  camddmw_arch.DAY_UNIT_DATA dat
         where  dat.Op_Date between vQuarterBeginDate and vQuarterEndDate
           and  dat.Unit_Id = any( vUnitIds )
         group
            by  dat.Unit_Id, 
