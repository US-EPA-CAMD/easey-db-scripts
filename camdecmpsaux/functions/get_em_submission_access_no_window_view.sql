...
CREATE OR REPLACE FUNCTION camdecmpsaux.get_em_submission_access_no_window_view(
    v_orisCode numeric,
    v_calendarYear integer,
    v_quarter integer
)
RETURNS TABLE (
    em_sub_access_id              numeric,
    mon_plan_id                   text,
    rpt_period_id                 integer,
...
BEGIN
    ...
    SELECT 
        ...
        vmp.mon_plan_id                 AS mon_plan_id,
        ...
        vmp.state                       AS state,
        vmp.facility_name               AS facility_name,
        rp.calendar_year                AS calendar_year,
        rp.quarter                      AS quarter,
        rp.period_abbreviation          AS period_abbreviation,
        rf.report_freq_cd               AS report_freq_cd,
    ...
    JOIN (
        SELECT 
            ...
            -- originally: camdecmpsaux.get_em_submission_status(mp.mon_plan_id, rp.calendar_year, rp.quarter)
            -- For now, just 'STATUS' or anything else to represent a sub_status
            'STATUS' AS em_sub_status
