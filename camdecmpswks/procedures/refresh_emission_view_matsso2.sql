...
-- DROP PROCEDURE camdecmpswks.refresh_emission_view_matsso2(character varying, numeric);
...
AS $BODY$
BEGIN
	DELETE FROM camdecmpswks.EMISSION_VIEW_MATS_SO2
		WHERE MON_PLAN_ID = vmonplanid AND RPT_PERIOD_ID = vrptperiodid;

	INSERT INTO camdecmpswks.EMISSION_VIEW_MATS_SO2
           (MON_PLAN_ID
           ,MON_LOC_ID
           ,RPT_PERIOD_ID
           ,DATE_HOUR
		...
		SELECT DISTINCT
				...
				SO2C_MHV.UNADJUSTED_HRLY_VALUE AS SO2_CONC_VALUE,		--	2015-11-24	EC-2454	MJ : Changed from MATS_MONITOR_HRLY_VALUE
				SO2C_MHV.MODC_CD AS SO2_CONC_MODC_CD,
				SO2C_MHV.PCT_AVAILABLE AS SO2_CONC_PMA,								--	2015-11-24	EC-2454	MJ : Changed from MATS_MONITOR_HRLY_VALUE 
				FLOW.UNADJUSTED_HRLY_VALUE AS FLOW_RATE,
				FLOW.MODC_CD AS FLOW_MODC,
				FLOW.PCT_AVAILABLE AS FLOW_PMA,
				CASE WHEN (DHV.PARAMETER_CD IN ('HCLRE', 'SO2RE', 'HGRE' )) THEN NULL
		...
		FROM temp_hourly_errors AS HOD 
				INNER JOIN camdecmpswks.MATS_DERIVED_HRLY_VALUE AS DHV ON DHV.HOUR_ID = HOD.HOUR_ID AND DHV.PARAMETER_CD IN ( 'SO2RE', 'SO2RH' )
				--		Chg 2015-11-24	EC-2454	MJ : From MATS_MONITOR_HRLY_VALUE To MONITOR_HRLY_VALUE
				LEFT OUTER JOIN camdecmpswks.MONITOR_HRLY_VALUE AS SO2C_MHV ON HOD.HOUR_ID = SO2C_MHV.HOUR_ID AND SO2C_MHV.PARAMETER_CD = 'SO2C'
