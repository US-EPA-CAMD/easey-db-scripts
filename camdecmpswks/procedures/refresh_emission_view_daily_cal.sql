...
BEGIN
	...
	SELECT DISTINCT
				...
				camdecmpswks.format_time( DC.UPSCALE_INJECTION_HOUR, DC.UPSCALE_INJECTION_MIN ) as UPSCALE_INJECTION_TIME,
				...
				camdecmpswks.format_time( DC.ZERO_INJECTION_HOUR, DC.ZERO_INJECTION_MIN ) as ZERO_INJECTION_TIME,
		...
		INNER JOIN camdecmpswks.DAILY_CALIBRATION AS DC
			ON DTS.DAILY_TEST_SUM_ID = DC.DAILY_TEST_SUM_ID
		LEFT OUTER JOIN camdecmpswks.MONITOR_SPAN AS MS 
			ON DTS.MON_LOC_ID = MS.MON_LOC_ID 
			AND DTS.COMPONENT_TYPE_CD = MS.COMPONENT_TYPE_CD 
			AND Coalesce(DTS.SPAN_SCALE_CD,'') = Coalesce(MS.SPAN_SCALE_CD,'')	-- both could be null, and null=null is not true!
			AND (camdecmpswks.EMISSIONS_MONITOR_SPAN_ACTIVE(MS.BEGIN_DATE, MS.BEGIN_HOUR, MS.END_DATE, MS.END_HOUR, DTS.END_DATE, CAST(LEFT(DTS.END_TIME, 2) AS numeric)) = 1)
