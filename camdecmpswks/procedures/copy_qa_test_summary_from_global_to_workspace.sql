-- PROCEDURE: camdecmpswks.copy_qa_test_from_global_to_workspace(numeric, numeric, numeric)

-- DROP PROCEDURE camdecmpswks.copy_qa_test_from_global_to_workspace(numeric, numeric, numeric);

CREATE OR REPLACE PROCEDURE camdecmpswks.copy_qa_test_from_global_to_workspace(
	oriscode numeric,
	vyear numeric,
	vquarter numeric)
LANGUAGE 'plpgsql'
...
BEGIN
	...
	IF vyear is null THEN 
		SELECT ARRAY(
			SELECT test_sum_id
			FROM camd.plant p
			JOIN camdecmps.monitor_plan mp on p.fac_id = mp.fac_id
			JOIN camdecmps.monitor_plan_location mpl on mp.mon_plan_id = mpl.mon_plan_id 
			JOIN camdecmps.test_summary ts on mpl.mon_loc_id = ts.mon_loc_id
			LEFT JOIN camdecmpsmd.reporting_period rp on ts.rpt_period_id = rp.rpt_period_id
			WHERE p.oris_code = orisCode
		) INTO testSumIds; 
	
	ELSE 
		IF oriscode is not null THEN 
			SELECT ARRAY(
				SELECT test_sum_id
				FROM camd.plant p
				JOIN camdecmps.monitor_plan mp on p.fac_id = mp.fac_id
				...
				JOIN camdecmps.test_summary ts on mpl.mon_loc_id = ts.mon_loc_id
				LEFT JOIN camdecmpsmd.reporting_period rp on ts.rpt_period_id = rp.rpt_period_id
				WHERE p.oris_code = orisCode AND ((rp.calendar_year = vyear and rp.quarter = vquarter) OR  (ts.end_date BETWEEN rptPeriod.begin_date AND rptPeriod.end_date))
			) INTO testSumIds; 
		ELSE
			SELECT ARRAY(
				SELECT test_sum_id
				...
				JOIN camdecmps.test_summary ts on mpl.mon_loc_id = ts.mon_loc_id
				LEFT JOIN camdecmpsmd.reporting_period rp on ts.rpt_period_id = rp.rpt_period_id
				WHERE ((rp.calendar_year = vyear and rp.quarter = vquarter) OR  (ts.end_date BETWEEN rptPeriod.begin_date AND rptPeriod.end_date))
			) INTO testSumIds; 
		END IF;
	END IF;
    ...
    DELETE FROM camdecmpswks.qa_supp_data
		WHERE test_sum_id = ANY(testSumIds);
  ...
  INSERT INTO camdecmpswks.qa_supp_data(
	...
	SELECT
		test_sum_id, mon_loc_id, mon_sys_id, component_id, test_num, gp_ind, calc_gp_ind, test_type_cd, test_reason_cd, test_result_cd, calc_test_result_cd, rpt_period_id, test_description, begin_date, begin_hour, begin_min, end_date, end_hour, end_min, calc_span_value, test_comment, last_updated, updated_status_flg, needs_eval_flg, chk_session_id, userid, add_date, update_date, span_scale_cd, injection_protocol_cd
	FROM camdecmps.test_summary
	WHERE test_sum_id = ANY(testSumIds);

	---------------------------------- Fuel Flowmeter Accuracy Data --------------------------------------------
	INSERT INTO camdecmpswks.fuel_flowmeter_accuracy(
	...
	SELECT
		fuel_flow_acc_id, test_sum_id, acc_test_method_cd, reinstall_date, reinstall_hour, low_fuel_accuracy, mid_fuel_accuracy, high_fuel_accuracy, userid, add_date, update_date
	FROM camdecmps.fuel_flowmeter_accuracy
	WHERE test_sum_id = ANY(testSumIds);

	---------------------------------- Transmitter Transducer Accuracy Data --------------------------------------------
	INSERT INTO camdecmpswks.trans_accuracy(
	...
	INSERT INTO camdecmpswks.fuel_flow_to_load_baseline(
	...
	SELECT
		fuel_flow_baseline_id, test_sum_id, accuracy_test_number, pei_test_number, avg_fuel_flow_rate, avg_load, baseline_fuel_flow_load_ratio, avg_hrly_hi_rate, baseline_ghr, nhe_cofiring, nhe_ramping, nhe_low_range, userid, add_date, update_date, fuel_flow_load_uom_cd, ghr_uom_cd
	FROM camdecmps.fuel_flow_to_load_baseline
	WHERE test_sum_id = ANY(testSumIds);

    ---------------------------------- Fuel Flow To Load Check --------------------------------------------
	INSERT INTO camdecmpswks.fuel_flow_to_load_check(
	...
	SELECT
		fuel_flow_load_id, test_sum_id, test_basis_cd, avg_diff, num_hrs, nhe_cofiring, nhe_ramping, nhe_low_range, userid, add_date, update_date
	FROM camdecmps.fuel_flow_to_load_check
	WHERE test_sum_id = ANY(testSumIds);

    ---------------------------------- AppendixE Correlation Test Summary Data --------------------------------------------
    INSERT INTO camdecmpswks.ae_correlation_test_sum(
    ...
    INSERT INTO camdecmpswks.ae_correlation_test_run(
	...
	WHERE ts.test_sum_id = ANY(testSumIds);

    INSERT INTO camdecmpswks.ae_hi_gas(
	...
	FROM camdecmps.ae_hi_gas ah
    JOIN camdecmps.ae_correlation_test_run ac ON ah.ae_corr_test_run_id = ac.ae_corr_test_run_id
    JOIN camdecmps.ae_correlation_test_sum ts ON ac.ae_corr_test_sum_id = ts.ae_corr_test_sum_id
	WHERE ts.test_sum_id = ANY(testSumIds);

    INSERT INTO camdecmpswks.ae_hi_oil(
	...
	FROM camdecmps.ae_hi_oil ah
    JOIN camdecmps.ae_correlation_test_run ac ON ah.ae_corr_test_run_id = ac.ae_corr_test_run_id
    JOIN camdecmps.ae_correlation_test_sum ts ON ac.ae_corr_test_sum_id = ts.ae_corr_test_sum_id
	WHERE ts.test_sum_id = ANY(testSumIds);

    ---------------------------------- Unit Default Test Data --------------------------------------------
    INSERT INTO camdecmpswks.unit_default_test(
	...
	SELECT
		unit_default_test_sum_id, test_sum_id, fuel_cd, operating_condition_cd, nox_default_rate, calc_nox_default_rate, group_id, num_units_in_group, num_tests_for_group, userid, add_date, update_date
	FROM camdecmps.unit_default_test
	WHERE test_sum_id = ANY(testSumIds);

    INSERT INTO camdecmpswks.unit_default_test_run(
	...
	FROM camdecmps.unit_default_test_run ur
    JOIN camdecmps.unit_default_test ud using(unit_default_test_sum_id)
	WHERE ud.test_sum_id = ANY(testSumIds);

    ---------------------------------- RATA ---------------------------------------------------------------
    INSERT INTO camdecmpswks.rata(
    ...
    INSERT INTO camdecmpswks.rata_summary(
	...
	WHERE r.test_sum_id = ANY(testSumIds);

    INSERT INTO camdecmpswks.rata_run(
    ...
    JOIN camdecmps.rata r ON r.rata_id = rs.rata_id
	WHERE r.test_sum_id = ANY(testSumIds);

    INSERT INTO camdecmpswks.flow_rata_run(
	...
	FROM camdecmps.flow_rata_run fr
    JOIN camdecmps.rata_run rr on fr.rata_run_id = rr.rata_run_id
    JOIN camdecmps.rata_summary rs ON rr.rata_sum_id = rs.rata_sum_id
    JOIN camdecmps.rata r ON r.rata_id = rs.rata_id
	WHERE r.test_sum_id = ANY(testSumIds);

    INSERT INTO camdecmpswks.rata_traverse(
	...
	FROM camdecmps.rata_traverse rt
    JOIN camdecmps.flow_rata_run fr ON fr.flow_rata_run_id = rt.flow_rata_run_id
    JOIN camdecmps.rata_run rr on fr.rata_run_id = rr.rata_run_id
    JOIN camdecmps.rata_summary rs ON rr.rata_sum_id = rs.rata_sum_id
    JOIN camdecmps.rata r ON r.rata_id = rs.rata_id
	WHERE r.test_sum_id = ANY(testSumIds);
    ...
    ---------------------------------- Linearity Summary -------------------------------------------------
    INSERT INTO camdecmpswks.linearity_summary(
	...
	SELECT
		lin_sum_id, test_sum_id, mean_ref_value, calc_mean_ref_value, mean_measured_value, calc_mean_measured_value, percent_error, calc_percent_error, aps_ind, calc_aps_ind, gas_level_cd, userid, add_date, update_date
	FROM camdecmps.linearity_summary
	WHERE test_sum_id = ANY(testSumIds);

    INSERT INTO camdecmpswks.linearity_injection(
	...
	FROM camdecmps.linearity_injection li
    JOIN camdecmps.linearity_summary ls using(lin_sum_id)
	WHERE ls.test_sum_id = ANY(testSumIds);

    ---------------------------------- Hg Linearity and 3-Level Summary Data -------------------------------------------------
    INSERT INTO camdecmpswks.hg_test_summary(
	...
	SELECT
		hg_test_sum_id, test_sum_id, gas_level_cd, mean_measured_value, mean_ref_value, percent_error, aps_ind, calc_mean_measured_value, calc_mean_ref_value, calc_percent_error, calc_aps_ind, userid, add_date, update_date
	FROM camdecmps.hg_test_summary
	WHERE test_sum_id = ANY(testSumIds);

    INSERT INTO camdecmpswks.hg_test_injection(
    ...
    JOIN camdecmps.hg_test_summary ts using(hg_test_sum_id)
	WHERE ts.test_sum_id = ANY(testSumIds);

    ---------------------------------- Cycle Time SummaryData -------------------------------------------------
    INSERT INTO camdecmpswks.cycle_time_summary(
	cycle_time_sum_id, test_sum_id, total_time, calc_total_time, userid, add_date, update_date)
	SELECT
		cycle_time_sum_id, test_sum_id, total_time, calc_total_time, userid, add_date, update_date
	FROM camdecmps.cycle_time_summary
	WHERE test_sum_id = ANY(testSumIds);

    INSERT INTO camdecmpswks.cycle_time_injection(
	...
	WHERE cts.test_sum_id = ANY(testSumIds);

    ---------------------------------- Online Offline Calibration Data -------------------------------------------------
    INSERT INTO camdecmpswks.on_off_cal(
    ...
    INSERT INTO camdecmpswks.flow_to_load_check(
	...
	SELECT
		flow_load_check_id, test_sum_id, test_basis_cd, avg_abs_pct_diff, num_hrs, nhe_fuel, nhe_ramping, nhe_bypass, nhe_pre_rata, nhe_test, nhe_main_bypass, bias_adjusted_ind, userid, add_date, update_date, op_level_cd
	FROM camdecmps.flow_to_load_check
	WHERE test_sum_id = ANY(testSumIds);

    ---------------------------------- Flow To Load Reference Data -------------------------------------------------
    INSERT INTO camdecmpswks.flow_to_load_reference(
	...
	SELECT
		flow_load_ref_id, test_sum_id, op_level_cd, avg_ref_method_flow, calc_avg_ref_method_flow, rata_test_num, avg_gross_unit_load, calc_avg_gross_unit_load, ref_flow_load_ratio, calc_ref_flow_load_ratio, avg_hrly_hi_rate, ref_ghr, calc_ref_ghr, userid, add_date, update_date, calc_sep_ref_ind
	FROM camdecmps.flow_to_load_reference
	WHERE test_sum_id = ANY(testSumIds);

    ---------------------------------- Test Qualification Data -------------------------------------------------
    INSERT INTO camdecmpswks.test_qualification(
	...
	SELECT
		test_qualification_id, test_sum_id, test_claim_cd, hi_load_pct, mid_load_pct, low_load_pct, begin_date, end_date, userid, add_date, update_date
	FROM camdecmps.test_qualification
	WHERE test_sum_id = ANY(testSumIds);

    ---------------------------------- Calibration Injection Data -------------------------------------------------
    INSERT INTO camdecmpswks.calibration_injection(
