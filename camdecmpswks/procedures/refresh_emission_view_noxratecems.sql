...
BEGIN
	...
	INNER JOIN camdecmpswks.DERIVED_HRLY_VALUE AS DHV 
		ON DHV.HOUR_ID = HOD.HOUR_ID AND DHV.PARAMETER_CD = 'NOXR' AND DHV.MODC_CD IS NOT NULL
	LEFT OUTER JOIN camdecmpswks.DERIVED_HRLY_VALUE AS NOX_DHV 
		ON HOD.HOUR_ID = NOX_DHV.HOUR_ID AND NOX_DHV.PARAMETER_CD = 'NOX' 
	LEFT OUTER JOIN camdecmpswks.DERIVED_HRLY_VALUE AS HI_DHV 
		ON HOD.HOUR_ID = HI_DHV.HOUR_ID AND HI_DHV.PARAMETER_CD = 'HI' 
	LEFT OUTER JOIN camdecmpswks.MONITOR_HRLY_VALUE AS NOXC_MHV 
		ON HOD.HOUR_ID = NOXC_MHV.HOUR_ID AND NOXC_MHV.PARAMETER_CD = 'NOXC' 
		AND NOXC_MHV.MODC_CD NOT IN ('47', '48')
	...
	LEFT OUTER JOIN camdecmpswks.MONITOR_FORMULA AS NOX_MF 
		ON NOX_DHV.MON_FORM_ID = NOX_MF.MON_FORM_ID
	LEFT OUTER JOIN camdecmpswks.MONITOR_HRLY_VALUE AS MHV_CO2C 
		ON DHV.HOUR_ID = MHV_CO2C.HOUR_ID AND MHV_CO2C.PARAMETER_CD = 'CO2C' 
		AND MHV_CO2C.MODC_CD NOT IN ('47', '48')
	LEFT OUTER JOIN camdecmpswks.MONITOR_HRLY_VALUE AS MHV_O2C_DRY 
		ON DHV.HOUR_ID = MHV_O2C_DRY.HOUR_ID AND MHV_O2C_DRY.PARAMETER_CD = 'O2C' 
		AND ((MHV_O2C_DRY.MOISTURE_BASIS IS NULL) OR (MHV_O2C_DRY.MOISTURE_BASIS = 'D')) 
		AND MHV_O2C_DRY.MODC_CD NOT IN ('47', '48') 
	LEFT OUTER JOIN camdecmpswks.MONITOR_HRLY_VALUE AS MHV_O2C_WET 
		ON DHV.HOUR_ID = MHV_O2C_WET.HOUR_ID AND MHV_O2C_WET.PARAMETER_CD = 'O2C' 
		AND ((MHV_O2C_WET.MOISTURE_BASIS IS NULL) OR (MHV_O2C_WET.MOISTURE_BASIS = 'W')) 
		AND MHV_O2C_WET.MODC_CD NOT IN ('47', '48')                              
