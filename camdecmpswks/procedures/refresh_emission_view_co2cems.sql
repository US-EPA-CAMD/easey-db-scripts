DROP PROCEDURE IF EXISTS camdecmpswks.refresh_emission_view_co2cems(text, numeric);
DROP PROCEDURE IF EXISTS camdecmpswks.refresh_emission_view_co2cems(character varying, numeric);
...
CALL camdecmpswks.load_temp_hourly_test_errors('MDC-94FE334B615548CDB3F22AED7D333EEF', 120);
CALL camdecmpswks.load_temp_hourly_test_errors('TWCORNEL5-C0E3879920A14159BAA98E03F1980A7A', 120);
CALL camdecmpswks.load_temp_hourly_test_errors('FPEARSON-P-718AA19AD70646A8A9C24BB36FE304C5', 120);
...
FROM (
	...
	FROM camdecmpswks.DERIVED_HRLY_VALUE
	WHERE RPT_PERIOD_ID = 120 AND PARAMETER_CD = 'CO2'
) AS dhv
JOIN (
	SELECT HOUR_ID, MON_LOC_ID, RPT_PERIOD_ID
	FROM camdecmpswks.MONITOR_HRLY_VALUE
