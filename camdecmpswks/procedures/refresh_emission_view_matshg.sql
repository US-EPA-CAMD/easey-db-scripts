-- PROCEDURE: camdecmpswks.refresh_emission_view_matshg(character varying, numeric)

DROP PROCEDURE IF EXISTS camdecmpswks.refresh_emission_view_matshg(character varying, numeric);
...
BEGIN
	...
	INNER JOIN camdecmpswks.MATS_MONITOR_HRLY_VALUE AS MHV 
		ON MHV.HOUR_ID = HOD.HOUR_ID AND MHV.PARAMETER_CD = 'HGC'
	INNER JOIN camdecmpswks.MATS_DERIVED_HRLY_VALUE AS DHV 
		ON DHV.HOUR_ID = HOD.HOUR_ID AND DHV.PARAMETER_CD IN ( 'HGRE', 'HGRH' )
	LEFT OUTER JOIN camdecmpswks.DERIVED_HRLY_VALUE AS H2OD 
		ON H2OD.HOUR_ID = HOD.HOUR_ID AND H2OD.PARAMETER_CD = 'H2O'
	LEFT OUTER JOIN camdecmpswks.MONITOR_FORMULA AS MF 
		ON MF.MON_FORM_ID = DHV.MON_FORM_ID
	LEFT OUTER JOIN camdecmpswks.MONITOR_HRLY_VALUE AS CO2C 
		ON CO2C.HOUR_ID = HOD.HOUR_ID AND CO2C.PARAMETER_CD = 'CO2C'
	LEFT OUTER JOIN camdecmpswks.MONITOR_HRLY_VALUE AS FLOW 
		ON FLOW.HOUR_ID = HOD.HOUR_ID AND FLOW.PARAMETER_CD = 'FLOW'
	LEFT OUTER JOIN camdecmpswks.MONITOR_HRLY_VALUE AS H2OM 
		ON H2OM.HOUR_ID = HOD.HOUR_ID AND H2OM.PARAMETER_CD = 'H2O'
	LEFT OUTER JOIN camdecmpswks.MONITOR_HRLY_VALUE AS O2D 
	...
	LEFT OUTER JOIN camdecmpswks.MONITOR_SYSTEM AS MSYS 
		ON MSYS.MON_SYS_ID = MHV.MON_SYS_ID
	LEFT OUTER JOIN camdecmpsmd.SYSTEM_TYPE_CODE AS SYS_TYP 
		ON SYS_TYP.SYS_TYPE_CD = MSYS.SYS_TYPE_CD
	LEFT OUTER JOIN camdecmpswks.HRLY_GAS_FLOW_METER GFM1 
		ON GFM1.HOUR_ID = HOD.HOUR_ID 
		AND GFM1.COMPONENT_ID = camdecmpswks.mats_sampling_train_component( MHV.MON_SYS_ID, HOD.BEGIN_DATE, HOD.BEGIN_HOUR, 1 )
	LEFT OUTER JOIN camdecmpswks.HRLY_GAS_FLOW_METER GFM2 
		ON GFM2.HOUR_ID = HOD.HOUR_ID 
		AND GFM2.COMPONENT_ID = camdecmpswks.mats_sampling_train_component( MHV.MON_SYS_ID, HOD.BEGIN_DATE, HOD.BEGIN_HOUR, 2 )
	LEFT OUTER JOIN camdecmpswks.COMPONENT CMP1 
