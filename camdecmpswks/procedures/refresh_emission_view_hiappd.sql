-- PROCEDURE: camdecmpswks.refresh_emission_view_hiappd(character varying, numeric)
...
BEGIN
	...
	INSERT INTO camdecmpswks.EMISSION_VIEW_HIAPPD
		...
		FROM temp_hourly_errors AS HOD 
				...
				INNER JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_HI ON ((HFF.HRLY_FUEL_FLOW_ID = HPFF_HI.HRLY_FUEL_FLOW_ID) AND (HPFF_HI.PARAMETER_CD = 'HI'))
				LEFT OUTER JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_GCV ON ((HFF.HRLY_FUEL_FLOW_ID = HPFF_GCV.HRLY_FUEL_FLOW_ID) AND (HPFF_GCV.PARAMETER_CD = 'GCV'))
				LEFT OUTER JOIN camdecmpswks.MONITOR_SYSTEM MS ON HFF.MON_SYS_ID = MS.MON_SYS_ID
				LEFT OUTER JOIN camdecmpswks.MONITOR_FORMULA AS MF ON HPFF_HI.MON_FORM_ID = MF.MON_FORM_ID
				LEFT OUTER JOIN camdecmpsmd.FUEL_CODE FC ON HFF.FUEL_CD = FC.FUEL_CD;
END
$BODY$;
