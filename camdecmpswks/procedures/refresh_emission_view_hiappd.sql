...
BEGIN
	...
	JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_HI 
		ON HFF.HRLY_FUEL_FLOW_ID = HPFF_HI.HRLY_FUEL_FLOW_ID
		AND HFF.MON_LOC_ID = HPFF_HI.MON_LOC_ID
		AND HFF.RPT_PERIOD_ID = HPFF_HI.RPT_PERIOD_ID
		AND HPFF_HI.PARAMETER_CD = 'HI'
	LEFT JOIN camdecmpswks.MONITOR_FORMULA AS MF 
		ON HPFF_HI.MON_FORM_ID = MF.MON_FORM_ID
	LEFT JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_GCV 
		ON HFF.HRLY_FUEL_FLOW_ID = HPFF_GCV.HRLY_FUEL_FLOW_ID
		AND HFF.MON_LOC_ID = HPFF_GCV.MON_LOC_ID
		AND HFF.RPT_PERIOD_ID = HPFF_GCV.RPT_PERIOD_ID
		AND HPFF_GCV.PARAMETER_CD = 'GCV'
