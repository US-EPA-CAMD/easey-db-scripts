...
BEGIN
	...
	LEFT JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_CO2 
	...
	LEFT JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_FC 	
	...
	LEFT JOIN camdecmpswks.MONITOR_FORMULA SUMMATION_MF
...
/*
  -- EXTREMELY INEFFICIENT WHEN LOTS OF DATA
	...
	INNER JOIN camdecmpswks.HRLY_FUEL_FLOW HFF 
		ON HOD.HOUR_ID = HFF.HOUR_ID
	LEFT OUTER JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_CO2 
		ON ((HFF.HRLY_FUEL_FLOW_ID = HPFF_CO2.HRLY_FUEL_FLOW_ID) AND (HPFF_CO2.PARAMETER_CD = 'CO2'))		
	LEFT OUTER JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_HI 
		ON ((HFF.HRLY_FUEL_FLOW_ID = HPFF_HI.HRLY_FUEL_FLOW_ID) AND (HPFF_HI.PARAMETER_CD = 'HI'))
	LEFT OUTER JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_FC 	
		ON ((HFF.HRLY_FUEL_FLOW_ID = HPFF_FC.HRLY_FUEL_FLOW_ID) AND (HPFF_FC.PARAMETER_CD = 'FC'))
	...
	LEFT OUTER JOIN camdecmpswks.MONITOR_FORMULA MF 
		ON HPFF_CO2.MON_FORM_ID = MF.MON_FORM_ID
	LEFT OUTER JOIN camdecmpswks.MONITOR_FORMULA SUMMATION_MF
		ON DHV_CO2.MON_FORM_ID = SUMMATION_MF.MON_FORM_ID
	WHERE HPFF_CO2.HRLY_FUEL_FLOW_ID IS NOT NULL OR (
		HPFF_CO2.HRLY_FUEL_FLOW_ID IS NULL AND (
			HPFF_HI.HRLY_FUEL_FLOW_ID IS NOT NULL AND DHV_CO2.PARAMETER_CD IS NOT NULL
