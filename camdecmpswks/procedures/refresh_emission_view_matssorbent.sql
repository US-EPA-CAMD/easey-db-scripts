-- PROCEDURE: camdecmpswks.refresh_emission_view_matssorbent(character varying, numeric)

DROP PROCEDURE IF EXISTS camdecmpswks.refresh_emission_view_matssorbent(character varying, numeric);
...
BEGIN
	...
	CREATE TEMP TABLE IF NOT EXISTS HourRules (HOUR_ID varchar(45) NOT NULL);
	INSERT INTO HourRules ( HOUR_ID ) 
...
WITH train (TRAP_ID, COMPONENT_ID, COMPONENT_IDENTIFIER, SORBENT_TRAP_SERIAL_NUMBER, MAIN_TRAP_HG, BREAKTHROUGH_TRAP_HG, SPIKE_TRAP_HG,
			SPIKE_REF_VALUE,
			TOTAL_SAMPLE_VOLUME,
...
AS
			(SELECT TRAP_ID, st.COMPONENT_ID, COMPONENT_IDENTIFIER, SORBENT_TRAP_SERIAL_NUMBER, MAIN_TRAP_HG, BREAKTHROUGH_TRAP_HG,
				SPIKE_TRAP_HG,
				SPIKE_REF_VALUE,
				TOTAL_SAMPLE_VOLUME,
				REF_FLOW_TO_SAMPLING_RATIO,
				...
				SAMPLE_DAMAGE_EXPLANATION
							 ...
							 JOIN camdecmpswks.MONITOR_PLAN_LOCATION mpl ON st.mon_loc_id=mpl.MON_LOC_ID
							 JOIN camdecmpswks.COMPONENT C ON C.COMPONENT_ID = st.COMPONENT_ID
	...
	INSERT INTO camdecmpswks.EMISSION_VIEW_MATSSORBENT
		...
		SELECT 	
			...
			camdecmpswks.format_date_hour(trp.END_DATE, trp.END_HOUR, null) AS end_date_time,
			...
			CASE WHEN hr.HOUR_ID IS NULL
				THEN NULL ELSE ts.ERROR_CODES END AS ERROR_CODES	--	EC-2393 M.Jones 2015-10-16
		FROM temp_hourly_errors ts
		INNER JOIN	camdecmpswks.SORBENT_TRAP trp  ON ts.MON_LOC_ID=trp.MON_LOC_ID AND trp.RPT_PERIOD_ID=ts.RPT_PERIOD_ID 
				AND trp.BEGIN_DATE=ts.BEGIN_DATE AND trp.BEGIN_HOUR=ts.BEGIN_HOUR
		INNER JOIN camdecmpswks.MONITOR_SYSTEM s ON trp.MON_SYS_ID=s.MON_SYS_ID
		LEFT OUTER JOIN HourRules hr ON hr.HOUR_ID = ts.HOUR_ID
		LEFT OUTER JOIN (SELECT DISTINCT TRAP_ID, MIN(COMPONENT_IDENTIFIER) AS ATrainID, MAX(COMPONENT_IDENTIFIER) AS BTrainID
							 FROM train
							 GROUP BY TRAP_ID) AS grouptrn ON trp.TRAP_ID=grouptrn.TRAP_ID
		LEFT OUTER JOIN train trnA ON trnA.TRAP_ID=grouptrn.TRAP_ID AND trnA.COMPONENT_IDENTIFIER=grouptrn.ATrainID
		LEFT OUTER JOIN train trnB ON trnB.TRAP_ID=grouptrn.TRAP_ID AND trnB.COMPONENT_IDENTIFIER=grouptrn.BTrainID;
END
$BODY$;
