...
-- DROP PROCEDURE camdecmpswks.refresh_emission_view_co2dailyfuel(character varying, numeric);
...
BEGIN
	...
	SELECT DISTINCT
			...
			CASE WHEN (Max(CASE WHEN rul.RULE_CHECK_ID IS NULL THEN 0 ELSE Coalesce(sev.SEVERITY_LEVEL, 0) END) > 0)
				 THEN 'View Errors'
				 ELSE null
			END AS ERROR_CODES
		FROM ( SELECT MON_PLAN_ID, MON_LOC_ID FROM camdecmpswks.MONITOR_PLAN_LOCATION WHERE MON_PLAN_ID = vmonplanid ) as mpl
				...
				INNER JOIN camdecmpswks.DAILY_EMISSION dem ON	dem.MON_LOC_ID = mpl.MON_LOC_ID AND dem.RPT_PERIOD_ID = evl.RPT_PERIOD_ID AND dem.PARAMETER_CD = 'CO2M'
				LEFT OUTER JOIN camdecmpswks.DAILY_FUEL df ON	dem.DAILY_EMISSION_ID = df.DAILY_EMISSION_ID
				...
				LEFT OUTER JOIN camdecmpsmd.CHECK_CATALOG_RESULT res ON	res.CHECK_CATALOG_RESULT_ID = log.CHECK_CATALOG_RESULT_ID
				LEFT OUTER JOIN camdecmpsmd.SEVERITY_CODE sev ON	sev.SEVERITY_CD = log.SEVERITY_CD
				LEFT OUTER JOIN camdecmpsmd.RULE_CHECK rul ON	rul.RULE_CHECK_ID = log.RULE_CHECK_ID AND rul.CATEGORY_CD = 'CO2DAY'
