...
CREATE OR REPLACE PROCEDURE camdecmpswks.refresh_emission_view_noxappemixedfuel(
...
BEGIN
	...
	INSERT INTO camdecmpswks.EMISSION_VIEW_NOXAPPEMIXEDFUEL
		...
		FROM temp_hourly_errors AS HOD 
				INNER JOIN DERIVED_HRLY_VALUE      DHV_NOXR ON ((DHV_NOXR.HOUR_ID = HOD.HOUR_ID) AND (DHV_NOXR.PARAMETER_CD = 'NOXR') AND (DHV_NOXR.OPERATING_CONDITION_CD IS NOT NULL))
				INNER JOIN MONITOR_SYSTEM          MS_NOXR  ON DHV_NOXR.MON_SYS_ID = MS_NOXR.MON_SYS_ID AND MS_NOXR.SYS_TYPE_CD='NOXE'
				LEFT OUTER JOIN DERIVED_HRLY_VALUE DHV_HI   ON ((DHV_HI.HOUR_ID = HOD.HOUR_ID) AND (DHV_HI.PARAMETER_CD = 'HI'))
				LEFT OUTER JOIN DERIVED_HRLY_VALUE DHV_NOX  ON ((DHV_NOX.HOUR_ID = HOD.HOUR_ID) AND (DHV_NOX.PARAMETER_CD = 'NOX'))
				LEFT OUTER JOIN MONITOR_FORMULA    MF_NOX   ON DHV_NOX.MON_FORM_ID = MF_NOX.MON_FORM_ID;
END
$BODY$;
