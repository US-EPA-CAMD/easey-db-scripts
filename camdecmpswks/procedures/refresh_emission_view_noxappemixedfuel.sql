-- PROCEDURE: camdecmpswks.refresh_emission_view_noxappemixedfuel(character varying, numeric)

DROP PROCEDURE IF EXISTS camdecmpswks.refresh_emission_view_noxappemixedfuel(character varying, numeric);
...
BEGIN
	...
	INSERT INTO camdecmpswks.EMISSION_VIEW_NOXAPPEMIXEDFUEL
		...
		FROM temp_hourly_errors AS HOD 
				INNER JOIN camdecmpswks.DERIVED_HRLY_VALUE      DHV_NOXR ON ((DHV_NOXR.HOUR_ID = HOD.HOUR_ID) AND (DHV_NOXR.PARAMETER_CD = 'NOXR') AND (DHV_NOXR.OPERATING_CONDITION_CD IS NOT NULL))
				INNER JOIN camdecmpswks.MONITOR_SYSTEM          MS_NOXR  ON DHV_NOXR.MON_SYS_ID = MS_NOXR.MON_SYS_ID AND MS_NOXR.SYS_TYPE_CD='NOXE'
