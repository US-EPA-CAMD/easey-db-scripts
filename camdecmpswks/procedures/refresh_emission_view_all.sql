-- PROCEDURE: camdecmpswks.refresh_emission_view_all(character varying, numeric)
...
BEGIN
	...
	LEFT JOIN camdecmpswks.MONITOR_FORMULA mf_HI
		ON dhv_HI.MON_FORM_ID = mf_HI.MON_FORM_ID
	LEFT JOIN camdecmpswks.DERIVED_HRLY_VALUE dhv_SO2
		ON dhv_SO2.HOUR_ID = hod.HOUR_ID
		AND dhv_SO2.MON_LOC_ID = hod.MON_LOC_ID
		AND dhv_SO2.RPT_PERIOD_ID = hod.RPT_PERIOD_ID
		AND dhv_SO2.PARAMETER_CD = 'SO2'
	LEFT JOIN camdecmpswks.MONITOR_FORMULA mf_SO2
		ON dhv_SO2.MON_FORM_ID = mf_SO2.MON_FORM_ID
	LEFT JOIN camdecmpswks.DERIVED_HRLY_VALUE dhv_NOXR
	...
	LEFT JOIN camdecmpswks.DERIVED_HRLY_VALUE dhv_CO2
	...
	LEFT JOIN camdecmpswks.MONITOR_FORMULA mf_CO2
		ON dhv_CO2.MON_FORM_ID = mf_CO2.MON_FORM_ID
	LEFT JOIN camdecmpswks.MONITOR_HRLY_VALUE AS FLOW_MHV
		ON FLOW_MHV.HOUR_ID = hod.HOUR_ID
		AND FLOW_MHV.MON_LOC_ID = hod.MON_LOC_ID
		AND FLOW_MHV.RPT_PERIOD_ID = hod.RPT_PERIOD_ID
		AND FLOW_MHV.PARAMETER_CD = 'FLOW'
	WHERE EXISTS (
		SELECT 1
		FROM camdecmpswks.DERIVED_HRLY_VALUE
		WHERE HOUR_ID = hod.HOUR_ID AND PARAMETER_CD IN ('HI', 'SO2', 'NOXR', 'NOX', 'CO2')
	) OR EXISTS (
		SELECT 1
...
/*
  -- EXTREMELY INEFFICIENT WHEN LOTS OF DATA
	SELECT DISTINCT	
		...
		hod.ERROR_CODES
	FROM temp_hourly_test_errors AS hod 
	LEFT OUTER JOIN camdecmpswks.DERIVED_HRLY_VALUE dhv_HI
		ON ((dhv_HI.HOUR_ID = hod.HOUR_ID) AND (dhv_HI.PARAMETER_CD = 'HI'))
	LEFT OUTER JOIN camdecmpswks.MONITOR_FORMULA mf_HI
		ON dhv_HI.MON_FORM_ID = mf_HI.MON_FORM_ID
	LEFT OUTER JOIN camdecmpswks.DERIVED_HRLY_VALUE dhv_SO2
		ON ((dhv_SO2.HOUR_ID = hod.HOUR_ID) AND (dhv_SO2.PARAMETER_CD = 'SO2'))
	LEFT OUTER JOIN camdecmpswks.MONITOR_FORMULA mf_SO2
		ON dhv_SO2.MON_FORM_ID = mf_SO2.MON_FORM_ID
	LEFT OUTER JOIN camdecmpswks.DERIVED_HRLY_VALUE dhv_NOXR
		ON ((dhv_NOXR.HOUR_ID = hod.HOUR_ID) AND (dhv_NOXR.PARAMETER_CD = 'NOXR'))
	LEFT OUTER JOIN camdecmpswks.MONITOR_FORMULA mf_NOXR
		ON dhv_NOXR.MON_FORM_ID = mf_NOXR.MON_FORM_ID
	LEFT OUTER JOIN camdecmpswks.DERIVED_HRLY_VALUE dhv_NOX
		ON ((dhv_NOX.HOUR_ID = hod.HOUR_ID) AND (dhv_NOX.PARAMETER_CD = 'NOX'))
	LEFT OUTER JOIN camdecmpswks.MONITOR_FORMULA mf_NOX
		ON dhv_NOX.MON_FORM_ID = mf_NOX.MON_FORM_ID
	LEFT OUTER JOIN camdecmpswks.DERIVED_HRLY_VALUE dhv_CO2
		ON ((dhv_CO2.HOUR_ID = hod.HOUR_ID) AND (dhv_CO2.PARAMETER_CD = 'CO2'))
	LEFT OUTER JOIN camdecmpswks.MONITOR_FORMULA mf_CO2
		ON dhv_CO2.MON_FORM_ID = mf_CO2.MON_FORM_ID
	LEFT OUTER JOIN camdecmpswks.MONITOR_HRLY_VALUE AS FLOW_MHV
