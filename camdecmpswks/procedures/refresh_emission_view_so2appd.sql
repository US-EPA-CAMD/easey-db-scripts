-- PROCEDURE: camdecmpswks.refresh_emission_view_so2appd(character varying, numeric)

DROP PROCEDURE IF EXISTS camdecmpswks.refresh_emission_view_so2appd(character varying, numeric);
...
BEGIN
	...
	INNER JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_SO2 
		ON ((HFF.HRLY_FUEL_FLOW_ID = HPFF_SO2.HRLY_FUEL_FLOW_ID) AND (HPFF_SO2.PARAMETER_CD = 'SO2'))
	LEFT OUTER JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_SULFUR 
		ON ((HFF.HRLY_FUEL_FLOW_ID = HPFF_SULFUR.HRLY_FUEL_FLOW_ID) AND (HPFF_SULFUR.PARAMETER_CD = 'SULFUR'))
	LEFT OUTER JOIN camdecmpswks.HRLY_PARAM_FUEL_FLOW HPFF_SO2R 
		ON ((HFF.HRLY_FUEL_FLOW_ID = HPFF_SO2R.HRLY_FUEL_FLOW_ID) AND (HPFF_SO2R.PARAMETER_CD = 'SO2R'))
	...
	LEFT OUTER JOIN camdecmpswks.DERIVED_HRLY_VALUE AS DHV_SO2 
