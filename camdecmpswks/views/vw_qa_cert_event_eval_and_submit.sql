...
AS
	SELECT
		...
		CASE
			...
			ELSE concat(qce.qa_cert_event_date, ' ', lpad(COALESCE(qce.qa_cert_event_hour, 0)::text, 2, '0'::text), ':00')
		END AS event_date,
		CASE
			WHEN qce.conditional_data_begin_date IS NULL THEN null::text
			ELSE concat(qce.conditional_data_begin_date, ' ', lpad(COALESCE(qce.conditional_data_begin_hour, 0)::text, 2, '0'::text), ':00')
		END AS condition_date,
		CASE
			WHEN qce.last_test_completed_date IS NULL THEN null::text
			ELSE concat(qce.last_test_completed_date, ' ', lpad(COALESCE(qce.last_test_completed_hour, 0)::text, 2, '0'::text), ':00')
