-- View: camdecmpswks.vw_mp_monitor_method_so2

DROP VIEW IF EXISTS camdecmpswks.vw_mp_monitor_method_so2;

CREATE OR REPLACE VIEW camdecmpswks.vw_mp_monitor_method_so2
 AS
 SELECT monitor_method.mon_method_id,
  ...
  WHERE monitor_method.parameter_cd::text = ANY (ARRAY['SO2'::character varying::text, 'SO2M'::character varying::text]);
