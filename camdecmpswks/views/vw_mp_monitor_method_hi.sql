-- View: camdecmpswks.vw_mp_monitor_method_hi

DROP VIEW IF EXISTS camdecmpswks.vw_mp_monitor_method_hi;

CREATE OR REPLACE VIEW camdecmpswks.vw_mp_monitor_method_hi
 AS
 SELECT monitor_method.mon_method_id,
  ...
  WHERE lower(monitor_method.parameter_cd::text) ~~ lower('HI%'::text);
