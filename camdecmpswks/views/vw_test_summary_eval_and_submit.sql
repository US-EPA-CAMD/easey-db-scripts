...
AS
	SELECT
		...
		CASE
			...
			ELSE concat(ts.begin_date, ' ', lpad(COALESCE(ts.begin_hour, 0)::text, 2, '0'::text), ':', lpad(COALESCE(ts.begin_min, 0)::text, 2, '0'::text))
		END AS begin_date,
		CASE
			WHEN ts.end_date IS NULL THEN null::text
			ELSE concat(ts.end_date, ' ', lpad(COALESCE(ts.end_hour, 0)::text, 2, '0'::text), ':', lpad(COALESCE(ts.end_min, 0)::text, 2, '0'::text))
		END AS end_date,
