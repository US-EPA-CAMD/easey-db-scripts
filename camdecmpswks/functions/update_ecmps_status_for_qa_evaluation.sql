				   ...
				   INSERT INTO tmpTestsStatus 
						SELECT TT.TEST_SUM_ID FROM (
							...
							(SELECT TS.MON_SYS_ID, TS.TEST_SUM_ID,
									YEAR(TS.END_DATE) AS CALENDAR_YEAR,
									FLOOR((MONTH(TS.END_DATE) + 2) / 3) AS QUARTER
								FROM camdecmpswks.TEST_SUMMARY TS
									INNER JOIN camdecmpswks.MONITOR_SYSTEM MS ON TS.MON_SYS_ID = MS.MON_SYS_ID
							...
							UNION SELECT DISTINCT T.TEST_SUM_ID
							FROM camdecmpswks.TEST_SUMMARY T
								INNER JOIN FUEL_FLOW_TO_LOAD_BASELINE B ON T.TEST_SUM_ID = B.TEST_SUM_ID,
	...
	/*		--	update EM evaluation
					INSERT INTO @tmpEmissionsStatus SELECT DISTINCT E.MON_PLAN_ID, E.RPT_PERIOD_ID
						FROM EMISSION_EVALUATION E,
							...
							CASE WHEN TS.RPT_PERIOD_ID IS NULL THEN YEAR(TS.END_DATE) ELSE RP.CALENDAR_YEAR END AS CALENDAR_YEAR,
							CASE WHEN TS.RPT_PERIOD_ID IS NULL THEN FLOOR((MONTH(TS.END_DATE) + 2) / 3) ELSE RP.QUARTER END AS QUARTER
						FROM TEST_SUMMARY TS
							LEFT OUTER JOIN REPORTING_PERIOD RP ON TS.RPT_PERIOD_ID = RP.RPT_PERIOD_ID) T
