-- FUNCTION: camdecmpswks.rpt_mp_load(character varying)

DROP FUNCTION IF EXISTS camdecmpswks.rpt_mp_load(character varying) CASCADE;
...
SELECT
		...
		camdecmpswks.format_indicator(l.second_normal_ind, true) AS "secondNormalIndicator",
		camdecmpswks.format_date_hour(l.load_analysis_date, null, null) AS "loadAnalysisDate",
		camdecmpswks.format_date_hour(l.begin_date, l.begin_hour, null) AS "beginDateHour",
		camdecmpswks.format_date_hour(l.end_date, l.end_hour, null) AS "endDateHour"
	FROM camdecmpswks.monitor_load l
