-- FUNCTION: camdecmpswks.ay_update_ecmps_status_for_mp_evaluation(character varying, character varying)

-- DROP FUNCTION IF EXISTS camdecmpswks.ay_update_ecmps_status_for_mp_evaluation(character varying, character varying);

CREATE OR REPLACE FUNCTION camdecmpswks.ay_update_ecmps_status_for_mp_evaluation(
...
begin
    ...
    if vSubmittable = 'Y' then     
       ...
       DELETE FROM camdecmpswks.CHECK_SESSION
	     ...
	     (SELECT qce.CHK_SESSION_ID 
           ...
           (SUBMISSION_AVAILABILITY_CD = 'REQUIRE' OR UPDATED_STATUS_FLG = 'Y') AND
			qce.MON_LOC_ID = mpl.MON_LOC_ID AND mpl.MON_PLAN_ID =vMonPlanId);
	 ...
	 UPDATE camdecmpswks.QA_CERT_EVENT
		SET NEEDS_EVAL_FLG = 'Y', 
		 ...
		 where NEEDS_EVAL_FLG = 'N' AND
           (SUBMISSION_AVAILABILITY_CD = 'REQUIRE' OR UPDATED_STATUS_FLG = 'Y') AND
		   MON_LOC_ID in (select MON_LOC_ID from camdecmpswks.MONITOR_PLAN_LOCATION
		   where MON_PLAN_ID =vMonPlanId);
    ...
    --------- wipe out calculated test data -----------
	 ...
	 create temp table tmpEmissionsStatus(MON_PLAN_ID character varying,RPT_PERIOD_ID int);
		...
		/* AY Q: Do we need table camdecmpsaux.EM_SUBMISSION_ACCESS here?
		I got it at line after @SUBMITTABLE = 'Y' 
		-- Should we use "ESA.SUBMISSION_AVAILABILITY_CD IN ('REQUIRE','GRANTED')" instead?
		*/
