-- FUNCTION: camdecmpswks.rpt_em_evaluation_trap_error_results(text, numeric, numeric)

DROP FUNCTION IF EXISTS camdecmpswks.rpt_em_evaluation_trap_error_results(text, numeric, numeric) CASCADE;

CREATE OR REPLACE FUNCTION camdecmpswks.rpt_em_evaluation_trap_error_results(
	vmonplanid text,
	vyear numeric,
	vquarter numeric)
...
SELECT DISTINCT
	...
	cl.severity_cd <> 'NONE' AND ccd.process_cd = 'HOURLY' AND cc.check_type_cd IN ('MATSTRN', 'MATSTRP');
$BODY$;
