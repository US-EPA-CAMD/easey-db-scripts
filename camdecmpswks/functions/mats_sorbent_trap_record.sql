...
DROP FUNCTION IF EXISTS camdecmpswks.mats_sorbent_trap_record(character varying, numeric);
...
select	COALESCE(unt.UNITID, stp.STACK_NAME) LOCATION_NAME,
          ...
          --  convert(int, dat.END_HOUR) END_HOUR,
            ...
            dat.SORBENT_TRAP_APS_CD,
  			camdecmpswks.Format_Date_Time(sys.BEGIN_DATE, sys.BEGIN_HOUR, 0) SYS_BEGIN_DATEHOUR,
			camdecmpswks.Format_Date_Time(sys.END_DATE, sys.END_HOUR, 0) SYS_END_DATEHOUR,
            dat.BORDER_TRAP_IND,
            dat.SUPP_DATA_IND,
            dat.MON_SYS_ID,
            dat.MON_LOC_ID,
