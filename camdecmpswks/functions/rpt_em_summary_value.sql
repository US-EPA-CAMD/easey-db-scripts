...
BEGIN
    ...
    RETURN QUERY
    SELECT
        camdecmpswks.get_config_by_loc_id(mon_loc_id) as "location",
        parameter_cd as "parameterCode",
		'Parameter Codes' AS "parameterCodeGroup",
		parameter_cd_description as "parameterCodeDescription",
        current_rpt_period_total as "currentRptPeriodTotal",
        calc_current_rpt_period_total as "calcCurrentRptPeriodTotal",
        os_total as "osTotal",
        calc_os_total as "calcOsTotal", 
        year_total as "yearTotal",
        calc_year_total as "calcYearTotal"
    FROM camdecmpswks.summary_value
	join camdecmpsmd.parameter_code using (parameter_cd)
    WHERE mon_loc_id = ANY (monLocIds) and rpt_period_id = rptperiodid; 
END;
$BODY$;

ALTER FUNCTION camdecmpswks.rpt_em_summary_value(text, numeric)
