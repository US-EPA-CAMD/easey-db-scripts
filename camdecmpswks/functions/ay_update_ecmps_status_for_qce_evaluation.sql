-- FUNCTION: camdecmpswks.ay_update_ecmps_status_for_qce_evaluation(character varying, character varying)

-- DROP FUNCTION IF EXISTS camdecmpswks.ay_update_ecmps_status_for_qce_evaluation(character varying, character varying);

CREATE OR REPLACE FUNCTION camdecmpswks.ay_update_ecmps_status_for_qce_evaluation(
			...
			INSERT INTO camdecmpswks.tmpEmissionsStatus 
				SELECT DISTINCT E.MON_PLAN_ID, E.RPT_PERIOD_ID
					...
					(SELECT MON_LOC_ID, QA_CERT_EVENT_ID,
							extract(year from camdecmpswks.QA_CERT_EVENT_DATE) AS CALENDAR_YEAR,
