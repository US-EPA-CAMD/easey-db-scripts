DROP FUNCTION IF EXISTS camdecmpswks.get_summary_values(text, character varying, numeric) CASCADE;
...
DROP FUNCTION IF EXISTS camdecmpswks.get_summary_values_pivoted(text, character varying, numeric) CASCADE;

CREATE OR REPLACE FUNCTION camdecmpswks.get_summary_values_pivoted(
	vtypecode text,
	vmonlocid character varying,
	vrptperiodid numeric
)
...
AS $BODY$
	...
	LEFT JOIN camdecmpswks.get_summary_values(vtypecode, sv.mon_loc_id, sv.rpt_period_id) ophr
		ON sv.mon_loc_id = ophr.mon_loc_id
		AND sv.rpt_period_id = ophr.rpt_period_id
		AND ophr.parameter_cd = 'OPHOURS'
	LEFT JOIN camdecmpswks.get_summary_values(vtypecode, sv.mon_loc_id, sv.rpt_period_id) optm
		ON sv.mon_loc_id = optm.mon_loc_id
		AND sv.rpt_period_id = optm.rpt_period_id
		AND optm.parameter_cd = 'OPTIME'
	LEFT JOIN camdecmpswks.get_summary_values(vtypecode, sv.mon_loc_id, sv.rpt_period_id) hit
		ON sv.mon_loc_id = hit.mon_loc_id
		AND sv.rpt_period_id = hit.rpt_period_id
		AND hit.parameter_cd = 'HIT'
	LEFT JOIN camdecmpswks.get_summary_values(vtypecode, sv.mon_loc_id, sv.rpt_period_id) so2m
		ON sv.mon_loc_id = so2m.mon_loc_id
		AND sv.rpt_period_id = so2m.rpt_period_id
		AND so2m.parameter_cd = 'SO2M'
	LEFT JOIN camdecmpswks.get_summary_values(vtypecode, sv.mon_loc_id, sv.rpt_period_id) co2m
		ON sv.mon_loc_id = co2m.mon_loc_id
		AND sv.rpt_period_id = co2m.rpt_period_id
		AND co2m.parameter_cd = 'CO2M'
	LEFT JOIN camdecmpswks.get_summary_values(vtypecode, sv.mon_loc_id, sv.rpt_period_id) noxr
