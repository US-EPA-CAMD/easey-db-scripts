...
DROP FUNCTION IF EXISTS camdecmpswks.rpt_em_evaluation_test_error_results(text, numeric, numeric) CASCADE;

CREATE OR REPLACE FUNCTION camdecmpswks.rpt_em_evaluation_test_error_results(
	vmonplanid text,
	vyear numeric,
	vquarter numeric)
    RETURNS TABLE("unitStack" text, "testTypeCode" text, "testDateTime" text, "sysCompId" text, "sysCompTypeScale" text, "severityCode" text, "categoryDescription" text, "checkCode" text, "resultMessage" text) 
    LANGUAGE 'sql'
...
SELECT DISTINCT
	...
	cl.severity_cd <> 'NONE' AND ccd.process_cd = 'HOURLY' AND cc.check_type_cd IN ('DAYCAL', 'EMTEST', 'EMWTS', 'EMWSI');
$BODY$;
