...
CREATE OR REPLACE FUNCTION camdecmpswks.rpt_test_extension_exemption_evaluation_results(
	testextensionexemptionids text[])
...
SELECT
	...
	WHERE rc.category_cd = 'TEE' AND ((
		cs.batch_id IS NULL AND cs.test_extension_exemption_id = ANY(testExtensionExemptionIds)
		...
		SELECT test_extension_exemption_id FROM camdecmpswks.check_session WHERE batch_id = (
			SELECT batch_id FROM camdecmpswks.check_session WHERE test_extension_exemption_id = ANY(testExtensionExemptionIds)
		)
	)));
$BODY$;