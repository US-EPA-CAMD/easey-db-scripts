-- FUNCTION: camdecmpswks.rpt_em_daily_backstop(text, numeric, numeric)

DROP FUNCTION IF EXISTS camdecmpswks.rpt_em_daily_backstop(text, numeric, numeric) CASCADE;

CREATE OR REPLACE FUNCTION camdecmpswks.rpt_em_daily_backstop(
	monplanid text,
	vyear numeric,
	vquarter numeric)
    RETURNS TABLE(
...
BEGIN
    ...
    RETURN QUERY
    SELECT
        camdecmpswks.get_config_by_loc_id(db.mon_loc_id) as "location",
        ...
        TO_CHAR(db.daily_noxm_exceed, 'FM999999999990.0') as "dailyNoxmExceed",
        TO_CHAR(db.cumulative_os_noxm_exceed, 'FM999999999990.0') as "cumulativeOsNoxmExceed"
    
    FROM camdecmpswks.daily_backstop db
