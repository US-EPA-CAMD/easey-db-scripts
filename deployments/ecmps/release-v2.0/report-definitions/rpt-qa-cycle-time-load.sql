...
----------------------------------------------------------------------------------------------------------------------
	...
	VALUES(datasetCode, 1, 'Cycle Time Test', 'SELECT * FROM {SCHEMA}.rpt_qa_cycle_time_summary($1, $2)', null)
...
----------------------------------------------------------------------------------------------------------------------
	/***** DATATABLE 2 *****/
	INSERT INTO camdaux.datatable(dataset_cd, table_order, display_name, sql_statement, no_results_msg_override)
	VALUES(datasetCode, 2, 'Cycle Time Injection', 'SELECT * FROM {SCHEMA}.rpt_qa_cycle_time_injection($1, $2)', null)
	RETURNING datatable_id INTO datatableId;
