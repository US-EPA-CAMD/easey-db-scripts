...
------------------------------------------------------------------------------------------------
	tableOrder := tableOrder + 1;
	DELETE FROM camdaux.datatable WHERE dataset_cd = datasetCode AND table_order = tableOrder AND template_cd = 'ERRDTL' AND sql_statement = 'SELECT * FROM {SCHEMA}.rpt_em_error_details($1, $2, $3)';
	INSERT INTO camdaux.datatable(dataset_cd, table_order, template_cd, sql_statement)
