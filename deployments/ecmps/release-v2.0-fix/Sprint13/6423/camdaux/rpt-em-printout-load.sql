...
-----------------------------------------------------------------------------------------------
	tableOrder := tableOrder + 1;
	DELETE FROM camdaux.datatable WHERE dataset_cd = datasetCode AND table_order = tableOrder AND template_cd = 'EMALL' AND sql_statement = 'SELECT * FROM {SCHEMA}.rpt_em_emission_view_all($1, $2, $3)';
	INSERT INTO camdaux.datatable(dataset_cd, table_order, template_cd, sql_statement)
...
-----------------------------------------------------------------------------------------------
	tableOrder := tableOrder + 1;
	DELETE FROM camdaux.datatable WHERE dataset_cd = datasetCode AND table_order = tableOrder AND template_cd = 'BACKSTOP' AND sql_statement = 'SELECT * FROM {SCHEMA}.rpt_em_daily_backstop($1, $2, $3)';
	INSERT INTO camdaux.datatable(dataset_cd, table_order, template_cd, sql_statement)
