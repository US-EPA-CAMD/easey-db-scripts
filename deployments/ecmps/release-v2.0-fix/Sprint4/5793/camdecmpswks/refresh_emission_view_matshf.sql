CREATE OR REPLACE PROCEDURE camdecmpswks.refresh_emission_view_matshf(IN vmonplanid character varying, IN vrptperiodid numeric)
 LANGUAGE plpgsql
AS $procedure$
DECLARE 
...
BEGIN
  ...
  RAISE NOTICE 'Deleting existing records...';
	DELETE FROM camdecmpswks.EMISSION_VIEW_MATSHF
	WHERE MON_PLAN_ID = vMonPlanId AND RPT_PERIOD_ID = vRptPeriodId;

  RAISE NOTICE 'Refreshing view data...';
	INSERT INTO camdecmpswks.EMISSION_VIEW_MATSHF(
		MON_PLAN_ID,
		MON_LOC_ID,
		RPT_PERIOD_ID,
		DATE_HOUR,
	...
	SELECT
		...
		mmhv.UNADJUSTED_HRLY_VALUE AS HF_CONC_VALUE, 
		mmhv.MODC_CD AS HF_CONC_MODC_CD,
		mmhv.PCT_AVAILABLE AS HF_CONC_PMA,
		...
		mdhv.UNADJUSTED_HRLY_VALUE AS RPT_HF_RATE,
		mdhv.CALC_UNADJUSTED_HRLY_VALUE AS CALC_HF_RATE,
		CASE 
	...
	JOIN camdecmpswks.MATS_DERIVED_HRLY_VALUE AS mdhv
		...
		AND mdhv.PARAMETER_CD IN ( 'HFRE', 'HFRH' )
  ...
  CALL camdecmpswks.refresh_emission_view_count(vmonplanid, vrptperiodid, 'MATSHF');
END
$procedure$
