CREATE OR REPLACE PROCEDURE camdecmpswks.refresh_emission_view_so2cems(IN vmonplanid character varying, IN vrptperiodid numeric)
 LANGUAGE plpgsql
AS $procedure$
declare 
...
BEGIN
  CALL camdecmpswks.load_temp_hourly_test_errors(vMonPlanId, vRptPeriodId);
	...
	SELECT DISTINCT
		...
		CASE (MF.EQUATION_CD) WHEN 'F-23' THEN Coalesce(DHV.SO2R_ADJUSTED_HRLY_VALUE, camdecmpswks.emissions_get_default_so2_rate(HOD.MON_LOC_ID, HOD.BEGIN_DATE, HOD.BEGIN_HOUR)) END AS DEFAULT_SO2_EMISSION_RATE,
		HOD.ERROR_CODES
