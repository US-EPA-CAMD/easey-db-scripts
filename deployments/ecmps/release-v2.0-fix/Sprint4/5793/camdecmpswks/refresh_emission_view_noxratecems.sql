CREATE OR REPLACE PROCEDURE camdecmpswks.refresh_emission_view_noxratecems(IN vmonplanid character varying, IN vrptperiodid numeric)
 LANGUAGE plpgsql
AS $procedure$
declare 
...
BEGIN
	...
	LEFT OUTER JOIN camdecmpswks.MONITOR_FORMULA AS NOX_MF 
	...
	LEFT OUTER JOIN camdecmpswks.MONITOR_DEFAULT AS DEF_CO2N 
		ON HOD.MON_LOC_ID = DEF_CO2N.MON_LOC_ID AND DEF_CO2N.DEFAULT_PURPOSE_CD = 'DC' AND DEF_CO2N.PARAMETER_CD = 'CO2N' 
		AND (camdecmpswks.emissions_monitor_default_active(DEF_CO2N.BEGIN_DATE, DEF_CO2N.BEGIN_HOUR, DEF_CO2N.END_DATE, DEF_CO2N.END_HOUR, HOD.BEGIN_DATE, HOD.BEGIN_HOUR) = 1)
