CREATE OR REPLACE PROCEDURE camdecmps.refresh_emission_view_co2appd(IN vmonplanid character varying, IN vrptperiodid numeric)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
  ...
  RAISE NOTICE 'Refreshing view data...';
	...
	LEFT JOIN camdecmps.get_hourly_param_fuel_flow_pivoted(
	  vmonplanid, vrptperiodid, hpffParamCodes
		...
		fc_mon_sys_id character varying,
	...
	where hpff.co2_hrly_fuel_flow_id is not null OR
		((hpff.co2_hrly_fuel_flow_id is null) and (hpff.hi_hrly_fuel_flow_id is not null or dhv.co2_hour_id is not null));
