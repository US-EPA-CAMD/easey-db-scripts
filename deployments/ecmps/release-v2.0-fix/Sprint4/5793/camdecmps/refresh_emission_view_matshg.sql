CREATE OR REPLACE PROCEDURE camdecmps.refresh_emission_view_matshg(IN vmonplanid character varying, IN vrptperiodid numeric)
 LANGUAGE plpgsql
AS $procedure$
DECLARE  
...
BEGIN
  ...
  RAISE NOTICE 'Refreshing view data...';
	...
	LEFT JOIN camdecmps.HRLY_GAS_FLOW_METER gfm1 
		...
		AND gfm1.COMPONENT_ID = camdecmps.mats_sampling_train_component( mmhv.MON_SYS_ID, hod.BEGIN_DATE, hod.BEGIN_HOUR, 1 )
	LEFT JOIN camdecmps.HRLY_GAS_FLOW_METER gfm2 
		...
		AND gfm2.COMPONENT_ID = camdecmps.mats_sampling_train_component( mmhv.MON_SYS_ID, hod.BEGIN_DATE, hod.BEGIN_HOUR, 2 )
