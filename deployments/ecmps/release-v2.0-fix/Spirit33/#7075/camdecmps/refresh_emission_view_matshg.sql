...
BEGIN
	...
	DELETE
      FROM  camdecmps.EMISSION_VIEW_MATSHG
                                        ...
                                        select  cmb.component_id,
                                          ...
                                          from  (
                                                    select  cmp.component_id,
                                                      ...
                                                      from  camdecmps.SORBENT_TRAP trp
                                                            join camdecmps.SAMPLING_TRAIN trn using ( trap_id )
                                                            join camdecmps.COMPONENT cmp using ( component_id )
                                                    ...
                                                    select  cmp.component_id,
                                                            cmp.component_identifier
                                                      from  camdecmps.SORBENT_TRAP_SUPP_DATA trp
                                                            join camdecmps.SAMPLING_TRAIN_SUPP_DATA trn using ( trap_id )
                                                            join camdecmps.COMPONENT cmp using ( component_id )
                                                     where  trp.mon_sys_id = hgc.mon_sys_id
                                                       and  ( hod.begin_date + interval '1 hour' * hod.begin_hour ) between ( trp.begin_date + interval '1 hour' * trp.begin_hour )
                                                                                                                        and ( trp.end_date + interval '1 hour' * trp.end_hour )
                                                       and  not exists
                                                            (
                                                                select  1
                                                                  from  camdecmps.SORBENT_TRAP exs
