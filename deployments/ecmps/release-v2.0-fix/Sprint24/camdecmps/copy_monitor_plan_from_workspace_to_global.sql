...
BEGIN
	...
	SELECT ARRAY(
		SELECT config_id
		FROM camdecmpswks.unit_stack_configuration AS usc, mp_timeframe
		WHERE (unit_id = ANY(unitIds)
		OR stack_pipe_id = ANY(stackPipeIds))
        AND mp_timeframe.begin_date >= usc.begin_date
        AND (
            usc.end_date IS NULL OR
            (mp_timeframe.end_date IS NOT NULL AND usc.end_date IS NOT NULL AND mp_timeframe.end_date <= usc.end_date)
        )
	...
	INSERT INTO camdecmpsaux.check_session (
			chk_session_id, mon_plan_id, test_sum_id, qa_cert_event_id, test_extension_exemption_id, rpt_period_id, session_begin_date, session_end_date, session_comment, severity_cd, category_cd, process_cd, userid, evaluation_id
	)
	SELECT
			cs.chk_session_id, cs.mon_plan_id, cs.test_sum_id, cs.qa_cert_event_id, cs.test_extension_exemption_id, cs.rpt_period_id, cs.session_begin_date, cs.session_end_date, cs.session_comment, cs.severity_cd, cs.category_cd, cs.process_cd, cs.userid, cs.evaluation_id
	FROM camdecmpswks.monitor_plan mp
