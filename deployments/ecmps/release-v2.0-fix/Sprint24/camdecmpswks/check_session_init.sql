...
-- FUNCTION: camdecmpswks.check_session_init(text, text, text, integer, text, text, text, date, date, text, text, bigint)
...
DROP FUNCTION IF EXISTS camdecmpswks.check_session_init(text, text, text, integer, text, text, text, date, date, text, text, bigint) CASCADE;
    ...
    -- do our insert now
	 INSERT INTO camdecmpswks.CHECK_SESSION
		   (CHK_SESSION_ID, PROCESS_CD,CATEGORY_CD,MON_PLAN_ID,RPT_PERIOD_ID,TEST_SUM_ID,QA_CERT_EVENT_ID,
			TEST_EXTENSION_EXEMPTION_ID, EVAL_BEGIN_DATE, EVAL_END_DATE, SESSION_COMMENT,USERID,LAST_UPDATED, BATCH_ID, SESSION_BEGIN_DATE, EVALUATION_ID)
		...
		NOW(), V_BATCH_ID, NOW(), V_EVALUATION_ID);
    END IF;
