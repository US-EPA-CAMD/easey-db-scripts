...
DECLARE
 ...
 select mon_plan_rf_id,  Mon_Plan_Id, First_Retire_Date,  begin_date, end_date, rpt_period_id
   from (
        ...
        join camdecmps.MONITOR_PLAN pln on pln.Mon_Plan_Id = frq.Mon_Plan_Id
        join camd.PLANT fac on fac.Fac_Id = pln.Fac_Id
       where  frq.End_Rpt_Period_Id is null
   and  ( exists
                (select  1 from  camdecmps.MONITOR_PLAN_LOCATION mpl
                           join  camdecmps.MONITOR_LOCATION loc on loc.Mon_Loc_Id = mpl.Mon_Loc_Id
                           join  camd.UNIT_OP_STATUS uos  on uos.Unit_Id = loc.Unit_Id  and uos.Op_Status_Cd = 'RET' and uos.End_Date is null
                            where  mpl.Mon_Plan_Id = pln.Mon_Plan_Id)
          or exists
...
BEGIN
    Open mprf_cursor;	
    LOOP
     fetch mprf_cursor into mprf_record; 
	  exit when not found;
       case when mprf_record.first_retire_date = mprf_record.begin_date then
            update camdecmps.monitor_plan_reporting_freq mprf set end_rpt_period_id = (mprf_record.rpt_period_id-1) 
              where mprf.MON_PLAN_ID=mprf_record.MON_PLAN_ID and mprf.mon_plan_rf_id=mprf_record.mon_plan_rf_id;
         when mprf_record.first_retire_date > mprf_record.begin_date then
            update camdecmps.monitor_plan_reporting_freq mprf set end_rpt_period_id = mprf_record.rpt_period_id
             where mprf.MON_PLAN_ID=mprf_record.MON_PLAN_ID and mprf.mon_plan_rf_id=mprf_record.mon_plan_rf_id;
       end case;
    END LOOP;
   close mprf_cursor;
