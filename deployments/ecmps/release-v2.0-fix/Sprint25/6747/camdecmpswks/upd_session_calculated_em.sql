-- DROP PROCEDURE camdecmpswks.upd_session_calculated_em(in varchar, in varchar, in int4, in varchar, inout bpchar, inout varchar);

CREATE OR REPLACE PROCEDURE camdecmpswks.upd_session_calculated_em(IN par_v_session_id character varying, IN par_v_mon_plan_id character varying, IN par_v_rpt_period_id integer, IN par_v_current_userid character varying, INOUT par_v_result character, INOUT par_v_error_msg character varying)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
    var_V_SUBMISSION_AVAILABILITY_CD VARCHAR(7);
--    var_V_MON_LOC_ID VARCHAR(45);
    var_V_MON_LOC_IDs varchar[];   
...
--            WHERE mpl.mon_plan_id = par_V_MON_PLAN_ID;
        ...
        /* Get mon_loc_id's */
        var_V_MON_LOC_IDs := ARRAY(
            SELECT mpl.mon_loc_id
            FROM camdecmpswks.monitor_plan_location AS mpl
            WHERE mpl.mon_plan_id = par_V_MON_PLAN_ID);
        RAISE NOTICE 'var_V_MON_LOC_IDs = %', var_V_MON_LOC_IDs;
        ...
        IF LOWER(var_V_SUBMISSION_AVAILABILITY_CD) = LOWER('REQUIRE') OR LOWER(var_V_SUBMISSION_AVAILABILITY_CD) = LOWER('GRANTED') THEN
           -- Operating Supp Data
            DELETE FROM camdecmpswks.operating_supp_data
                WHERE rpt_period_id = par_V_RPT_PERIOD_ID AND mon_loc_id = any(var_V_MON_LOC_IDs);
            ...
            SELECT
                ...
                WHERE src.rpt_period_id = par_V_RPT_PERIOD_ID AND src.mon_loc_id = any(var_V_MON_LOC_IDs);
