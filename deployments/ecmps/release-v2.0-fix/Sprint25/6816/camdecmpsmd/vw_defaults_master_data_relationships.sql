...
DROP VIEW IF EXISTS camdecmpsmd.vw_defaults_master_data_relationships;
 ...
 SELECT param_code.parameter_code,
   ...
   FROM ( SELECT param.value1 AS parameter_code,
     ...
     LEFT JOIN ( SELECT vw_cross_check_catalog_value.value1 AS parameter_code,
           ...
           FROM camdecmpsmd.vw_cross_check_catalog_value
          WHERE vw_cross_check_catalog_value.cross_chk_catalog_name::text = 'Default Parameter to Purpose'::text) purp ON param_code.parameter_code = purp.parameter_code
     LEFT JOIN ( SELECT vw_cross_check_catalog_value.value1 AS parameter_code,
     ...
     LEFT JOIN ( SELECT vw_cross_check_catalog_value.value1 AS parameter_code,
           ...
           FROM camdecmpsmd.vw_cross_check_catalog_value
          WHERE vw_cross_check_catalog_value.cross_chk_catalog_name::text = 'Default Parameter to Source of Value'::text) src ON param_code.parameter_code = src.parameter_code;
