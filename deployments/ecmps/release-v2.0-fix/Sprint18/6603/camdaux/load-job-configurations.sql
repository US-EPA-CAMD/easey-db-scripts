...
values  ('PdemJob', 'MAINTAINANCE', 'Program Data Emissions Job', 'Operates on an interval to determine if emission reports in PDEM_REPORT table need to be generated.', 'Program Data Emissions Job Queue Trigger', 'Operate every minute to determine if there are PDEM reports which can be triggered for generation', '0 0/1 * 1/1 * ? *', TRUE, FALSE, NULL);
