-- FUNCTION: camdecmpswks.rpt_em_emission_summary(text, text, text)

-- DROP FUNCTION IF EXISTS camdecmpswks.rpt_em_emission_summary(text, text, text);

CREATE OR REPLACE FUNCTION camdecmpswks.rpt_em_emission_summary(
	p_mon_plan_id text,
	p_loc_id text,
...
BEGIN
			 ...
			 from camdecmps.SUMMARY_VALUE smv
 			 join camdecmps.EMISSION_EVALUATION ems on ems.rpt_period_id =smv.rpt_period_id
  			 join camdecmps.MONITOR_PLAN_LOCATION mpl on mpl.mon_plan_id=ems.mon_plan_id 
   				  and mpl.mon_loc_id=smv.mon_loc_id  
 			 join camdecmpsmd.REPORTING_PERIOD prs on prs.rpt_period_id = smv.rpt_period_id
  			 join ( select 'QTRRPT' as summary_type_cd, 'Quarterly Reported' as summary_name, 1 as summary_num union all
        		 ...
        		 select 'OSRPT' as summary_type_cd, 'Ozone Season Reported' as summary_name, 5 as summary_num union all
         		 select 'OSCALC' as summary_type_cd, 'Ozone Season Calculated' as summary_name, 6 as summary_num
                      ...
                      on null is null
             where smv.parameter_cd in ('CO2M','HIT','NOXM','NOXR','OPHOURS','OPTIME','SO2M') 
		  	 and  mpl.mon_plan_id= p_mon_plan_id
		   	 and  smv.rpt_period_id = p_rpt_period_id::numeric
 ...
 union  all
   select  lst.period_description, lst.summary_name as row_name,
      ...
      group by  lst.mon_loc_id, lst.rpt_period_id,lst.period_description,
	            lst.summary_num, lst.summary_type_cd, lst.summary_name
        ) cmb
 order
    by  period_description,
        summary_num;
END;
$BODY$;

GRANT EXECUTE ON FUNCTION camdecmpswks.rpt_em_emission_summary(text, text, text) TO PUBLIC;

GRANT EXECUTE ON FUNCTION camdecmpswks.rpt_em_emission_summary(text, text, text) TO "r_emissions-api";

GRANT EXECUTE ON FUNCTION camdecmpswks.rpt_em_emission_summary(text, text, text) TO "r_qa-cert-api";

GRANT EXECUTE ON FUNCTION camdecmpswks.rpt_em_emission_summary(text, text, text) TO "r_quartz-scheduler";

GRANT EXECUTE ON FUNCTION camdecmpswks.rpt_em_emission_summary(text, text, text) TO uryp8x9bvx7lbph4;
