...
BEGIN
    ...
    DELETE FROM camdecmps.derived_hrly_value
    WHERE mon_loc_id = ANY(monLocIds) and rpt_period_id = rptperiodid;

    DELETE FROM camdecmps.hrly_param_fuel_flow
    WHERE mon_loc_id = ANY(monLocIds) and rpt_period_id = rptperiodid;

    DELETE FROM camdecmps.hrly_fuel_flow
    WHERE mon_loc_id = ANY(monLocIds) and rpt_period_id = rptperiodid;

    DELETE FROM camdecmps.hrly_gas_flow_meter
    WHERE mon_loc_id = ANY(monLocIds) and rpt_period_id = rptperiodid;

    DELETE FROM camdecmps.mats_derived_hrly_value
    WHERE mon_loc_id = ANY(monLocIds) and rpt_period_id = rptperiodid;

    DELETE FROM camdecmps.mats_monitor_hrly_value
    WHERE mon_loc_id = ANY(monLocIds) and rpt_period_id = rptperiodid;

    DELETE FROM camdecmps.monitor_hrly_value
    WHERE mon_loc_id = ANY(monLocIds) and rpt_period_id = rptperiodid;
