DROP FUNCTION IF EXISTS camdecmps.get_qatest_feedback(text) CASCADE;

CREATE OR REPLACE FUNCTION camdecmps.get_qatest_feedback(testId text)
RETURNS TABLE(
...
SELECT
    ...
    CASE
        ...
        ELSE camdecmps.format_date_hour(ts.end_date, ts.end_hour, ts.end_min)
        END AS end_test
FROM camdecmps.test_summary ts
    ...
    WHERE cl.severity_cd <> 'NONE' OR cl.severity_cd IS NULL
         ...
         INNER JOIN camdecmps.vw_monitor_location ml ON ts.mon_loc_id = ml.mon_loc_id
         LEFT OUTER JOIN camdecmpsmd.reporting_period rp ON ts.rpt_period_id = rp.rpt_period_id
         LEFT OUTER JOIN camdecmps.component c ON ts.component_id = c.component_id
         LEFT OUTER JOIN camdecmps.monitor_system ms ON ts.mon_sys_id = ms.mon_sys_id
