...
------------------------------------------------------------------------------------------------
	...
	VALUES(datasetCode, tableOrder, 'FACINFO1C', 'SELECT * FROM {SCHEMA}.rpt_facility_information($1, $2)')
...
------------------------------------------------------------------------------------------------
	...
	tableOrder := tableOrder + 1;
	INSERT INTO camdaux.datatable(dataset_cd, table_order, template_cd, sql_statement)
	VALUES(datasetCode, tableOrder, 'UNITSTACK', 'SELECT * FROM {SCHEMA}.rpt_mp_unit_stack_configuration($1)')
	RETURNING datatable_id INTO datatableId;

	/***** COLUMNS *****/
	INSERT INTO camdaux.datacolumn(datatable_id, column_order, name, display_name)
	VALUES
		( datatableId, 1, 'stackPipeIdentifier', 'Stack/Pipe Identifier' ),
		( datatableId, 2, 'unitIdentifier',      'Unit Identifier' ),
...
------------------------------------------------------------------------------------------------
	...
	VALUES
		...
		(datatableId, 6, 'endDate', 'End Date');
...
------------------------------------------------------------------------------------------------
	...
	tableOrder := tableOrder + 1;
	INSERT INTO camdaux.datatable(dataset_cd, table_order, template_cd, sql_statement)
	VALUES(datasetCode, tableOrder, 'UNITCONTROL', 'SELECT * FROM {SCHEMA}.rpt_mp_unit_control($1)')
	RETURNING datatable_id INTO datatableId;
	...
	VALUES
		...
		(datatableId, 3, 'controlEquipmentCode', 'Equipment Code'),
		(datatableId, 4, 'originalEquipment', 'Original Equipment'),
		(datatableId, 5, 'ozoneSeasonControl', 'OS Control'),
		(datatableId, 6, 'installDate', 'Install Date'),
		(datatableId, 7, 'optimizationDate', 'Optimization Date'),
		(datatableId, 8, 'retireDate', 'Retire Date'),
		(datatableId, 9, 'status', 'Status');
...
------------------------------------------------------------------------------------------------
	...
	VALUES
		...
		(datatableId,  4, 'fuelCode', 'Fuel Code'),
		(datatableId,  5, 'systemDesignation', 'System Designation'),
		(datatableId,  6, 'systemBeginDateHour', 'Begin Date/Hr'),
		(datatableId,  7, 'systemEndDateHour', 'End Date/Hr'),
		(datatableId,  8, 'componentIdentifier', 'Component Id'),
		(datatableId,  9, 'componentTypeCode', 'Component Type Code'),
		(datatableId, 10, 'acquisitionMethodCode', 'Acquisition Method Code'),
		(datatableId, 11, 'basis', 'Basis Code'),
		(datatableId, 12, 'manufacturer', 'Manufacturer'),
		(datatableId, 13, 'modelVersion', 'Model/Version'),
		(datatableId, 14, 'serialNumber', 'Serial Number'),
		(datatableId, 15, 'componentBeginDateHour', 'Begin Date/Hr'),
		(datatableId, 16, 'componentEndDateHour', 'End Date/Hr'),
		(datatableId, 17, 'hgConverterIndicator', 'Hg Converter Indicator');
...
------------------------------------------------------------------------------------------------
	...
	tableOrder := tableOrder + 1;
	INSERT INTO camdaux.datatable(dataset_cd, table_order, template_cd, sql_statement)
	VALUES(datasetCode, tableOrder, 'RECTDUCTWAF', 'SELECT * FROM {SCHEMA}.rpt_mp_waf_duct_rect($1)')
	RETURNING datatable_id INTO datatableId;
	...
	VALUES
		(datatableId,  1, 'unitStack', 'Unit/Stack'),
		(datatableId,  2, 'wafMethod', 'Method'),
		(datatableId,  3, 'wafValue', 'Value'),
		(datatableId,  4, 'numTestRuns', 'Test Runs'),
		(datatableId,  5, 'numTraversePointsWaf', 'Traverse Points WAF'),
		(datatableId,  6, 'numTestPorts', 'Test Ports'),
		(datatableId,  7, 'numTraversePointsRef', 'Traverse Points Ref'),
		(datatableId,  8, 'ductWidth', 'Duct Width'),
		(datatableId,  9, 'ductDepth', 'Duct Depth'),
		(datatableId, 10, 'determinationDate', 'Determination Date'),
		(datatableId, 11, 'effectiveDateHour', 'Effective Date/Hr'),
		(datatableId, 12, 'endDateHour', 'End Date/Hr');
