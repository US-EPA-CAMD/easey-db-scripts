-- FUNCTION: camdecmps.rpt_mp_waf_duct_rect(character varying)

CREATE OR REPLACE FUNCTION camdecmps.rpt_mp_waf_duct_rect
(
...
AS $BODY$
    ...
    select  coalesce( unt.unitid, stp.stack_name ) as "unitStack",
            ...
            camdecmps.format_date_hour( waf.waf_effective_date , waf.waf_effective_hour , null ) as "effectiveDateHour",
            camdecmps.format_date_hour( waf.end_date, waf.end_hour, null )  as "endDateHour"
      from  camdecmps.MONITOR_PLAN_LOCATION mpl
            join camdecmps.RECT_DUCT_WAF waf using ( mon_loc_id )
