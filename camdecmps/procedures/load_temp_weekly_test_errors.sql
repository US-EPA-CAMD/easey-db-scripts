-- PROCEDURE: camdecmps.load_temp_weekly_test_errors(character varying, numeric)

DROP PROCEDURE IF EXISTS camdecmps.load_temp_weekly_test_errors(character varying, numeric);
  ...
  IF NOT EXISTS (
    ...
    INSERT INTO temp_weekly_test_errors(
      ...
      SELECT DISTINCT 
          ...
          wts.SPAN_SCALE_CD, 
          wts.TEST_DATE, 
          camdecmps.format_time(TEST_HOUR, TEST_MIN) as TEST_TIME,
          wts.TEST_TYPE_CD, 
          wts.TEST_RESULT_CD, 
    ...
    FROM (
      ...
      GROUP BY
          ...
          wts.SPAN_SCALE_CD, 
          wts.TEST_DATE, 
          camdecmps.format_time(TEST_HOUR, TEST_MIN),
          wts.TEST_TYPE_CD, 
          wts.TEST_RESULT_CD, 
          wts.CALC_TEST_RESULT_CD;
	END IF;
