-- PROCEDURE: camdecmps.refresh_emission_view_co2cems(character varying, numeric)
...
BEGIN
	...
	INNER JOIN camdecmps.DERIVED_HRLY_VALUE AS DHV
		...
		AND DHV.PARAMETER_CD = 'CO2'
	INNER JOIN camdecmps.MONITOR_HRLY_VALUE AS FLOW_MHV
		ON HOD.HOUR_ID = FLOW_MHV.HOUR_ID
		AND HOD.MON_LOC_ID = FLOW_MHV.MON_LOC_ID
		AND HOD.RPT_PERIOD_ID = FLOW_MHV.RPT_PERIOD_ID
		AND FLOW_MHV.PARAMETER_CD = 'FLOW' 
	...
	LEFT JOIN camdecmps.DERIVED_HRLY_VALUE AS CO2C_DHV
		ON HOD.HOUR_ID = CO2C_DHV.HOUR_ID
		AND HOD.MON_LOC_ID = CO2C_DHV.MON_LOC_ID
		AND HOD.RPT_PERIOD_ID = CO2C_DHV.RPT_PERIOD_ID
		AND CO2C_DHV.PARAMETER_CD = 'CO2C'
	LEFT JOIN camdecmps.MONITOR_HRLY_VALUE AS CO2C_MHV
		ON HOD.HOUR_ID = CO2C_MHV.HOUR_ID
		AND HOD.MON_LOC_ID = CO2C_MHV.MON_LOC_ID
		AND HOD.RPT_PERIOD_ID = CO2C_MHV.RPT_PERIOD_ID
		AND CO2C_MHV.PARAMETER_CD = 'CO2C'
...
/*
  -- EXTREMELY INEFFICIENT WHEN LOTS OF DATA
	SELECT DISTINCT
		HOD.MON_PLAN_ID, 
		HOD.MON_LOC_ID, 
		HOD.RPT_PERIOD_ID, 
	...
	FROM temp_hourly_test_errors AS HOD 
	INNER JOIN camdecmps.DERIVED_HRLY_VALUE AS DHV 
		ON DHV.HOUR_ID = HOD.HOUR_ID AND DHV.PARAMETER_CD = 'CO2'
	INNER JOIN camdecmps.MONITOR_HRLY_VALUE AS FLOW_MHV 
		ON HOD.HOUR_ID = FLOW_MHV.HOUR_ID AND FLOW_MHV.PARAMETER_CD = 'FLOW' 
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE AS CO2M_DHV 
		ON CO2M_DHV.HOUR_ID = HOD.HOUR_ID AND CO2M_DHV.PARAMETER_CD = 'CO2M'
	LEFT OUTER JOIN camdecmps.MONITOR_FORMULA AS MF 
		ON DHV.MON_FORM_ID = MF.MON_FORM_ID 
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE AS CO2C_DHV 
		ON CO2C_DHV.HOUR_ID = HOD.HOUR_ID AND CO2C_DHV.PARAMETER_CD = 'CO2C'  
	LEFT OUTER JOIN camdecmps.MONITOR_HRLY_VALUE AS CO2C_MHV 
		ON CO2C_MHV.HOUR_ID = HOD.HOUR_ID AND CO2C_MHV.PARAMETER_CD = 'CO2C'  
	LEFT OUTER JOIN camdecmps.MONITOR_HRLY_VALUE AS H2O_MHV 
		ON DHV.HOUR_ID = H2O_MHV.HOUR_ID AND H2O_MHV.PARAMETER_CD = 'H2O' 
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE AS H2O_DHV 
		ON DHV.HOUR_ID = H2O_DHV.HOUR_ID AND H2O_DHV.PARAMETER_CD = 'H2O' 
	LEFT OUTER JOIN camdecmps.MONITOR_DEFAULT AS H2O_MD 
		ON HOD.MON_LOC_ID = H2O_MD.MON_LOC_ID AND H2O_MD.PARAMETER_CD = 'H2O' AND H2O_MD.DEFAULT_PURPOSE_CD = 'PM' AND
		camdecmps.emissions_monitor_default_active(H2O_MD.BEGIN_DATE, H2O_MD.BEGIN_HOUR, H2O_MD.END_DATE, H2O_MD.END_HOUR, HOD.BEGIN_DATE, HOD.BEGIN_HOUR) = 1;
