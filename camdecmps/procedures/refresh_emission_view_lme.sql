-- PROCEDURE: camdecmps.refresh_emissiON_view_lme()

DROP PROCEDURE IF EXISTS camdecmps.refresh_emissiON_view_lme();

CREATE OR REPLACE PROCEDURE camdecmps.refresh_emissiON_view_lme()
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
	TRUNCATE camdecmps.EMISSION_VIEW_LME RESTART IDENTITY;
	...
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE AS HIT_DHV 
		ON HOD.HOUR_ID = HIT_DHV.HOUR_ID AND HIT_DHV.PARAMETER_CD = 'HIT'
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE AS SO2M_DHV 
		ON HOD.HOUR_ID = SO2M_DHV.HOUR_ID AND SO2M_DHV.PARAMETER_CD = 'SO2M'
	LEFT OUTER JOIN camdecmps.MONITOR_DEFAULT as SO2R_MD 
		ON HOD.MON_LOC_ID = SO2R_MD.MON_LOC_ID AND 
		camdecmps.emissions_monitor_default_active(SO2R_MD.BEGIN_DATE, SO2R_MD.BEGIN_HOUR, SO2R_MD.END_DATE, SO2R_MD.END_HOUR, HOD.BEGIN_DATE, HOD.BEGIN_HOUR) = 1 
	...
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE AS NOXM_DHV 
		ON HOD.HOUR_ID = NOXM_DHV.HOUR_ID AND NOXM_DHV.PARAMETER_CD = 'NOXM'
	LEFT OUTER JOIN camdecmps.MONITOR_DEFAULT AS NOXR_MD 
	...
	LEFT OUTER JOIN camdecmps.MONITOR_DEFAULT AS NORX_MD 
	...
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE as CO2M_DHV 
		ON HOD.HOUR_ID = CO2M_DHV.HOUR_ID AND CO2M_DHV.PARAMETER_CD = 'CO2M'
	LEFT OUTER JOIN camdecmps.MONITOR_DEFAULT AS CO2R_MD 
		ON HOD.MON_LOC_ID = CO2R_MD.MON_LOC_ID AND 
		camdecmps.emissions_monitor_default_active(CO2R_MD.BEGIN_DATE, CO2R_MD.BEGIN_HOUR, CO2R_MD.END_DATE, CO2R_MD.END_HOUR, HOD.BEGIN_DATE, HOD.BEGIN_HOUR) = 1 
	...
	WHERE HOD.HOUR_ID IN (
		SELECT HOUR_ID
		FROM camdecmps.DERIVED_HRLY_VALUE
		WHERE PARAMETER_CD IN ('SO2M', 'NOXM', 'CO2M','HIT')
	);
END
$BODY$;
