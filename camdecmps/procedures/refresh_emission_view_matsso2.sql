-- PROCEDURE: camdecmps.refresh_emission_view_matsso2()

DROP PROCEDURE IF EXISTS camdecmps.refresh_emission_view_matsso2();

CREATE OR REPLACE PROCEDURE camdecmps.refresh_emission_view_matsso2()
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
	TRUNCATE camdecmps.EMISSION_VIEW_MATSSO2 RESTART IDENTITY;
	...
	INNER JOIN camdecmps.MATS_DERIVED_HRLY_VALUE AS DHV 
		ON DHV.HOUR_ID = HOD.HOUR_ID AND DHV.PARAMETER_CD IN ( 'SO2RE', 'SO2RH' )
	LEFT OUTER JOIN camdecmps.MONITOR_HRLY_VALUE AS SO2C_MHV 
		ON HOD.HOUR_ID = SO2C_MHV.HOUR_ID AND SO2C_MHV.PARAMETER_CD = 'SO2C'
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE AS H2OD 
	...
	LEFT OUTER JOIN camdecmps.MONITOR_HRLY_VALUE AS CO2C 
		ON CO2C.HOUR_ID = HOD.HOUR_ID AND CO2C.PARAMETER_CD = 'CO2C'
	LEFT OUTER JOIN camdecmps.MONITOR_HRLY_VALUE AS FLOW 
		ON FLOW.HOUR_ID = HOD.HOUR_ID AND FLOW.PARAMETER_CD = 'FLOW'
	LEFT OUTER JOIN camdecmps.MONITOR_HRLY_VALUE AS H2OM 
		ON H2OM.HOUR_ID = HOD.HOUR_ID AND H2OM.PARAMETER_CD = 'H2O'
	LEFT OUTER JOIN camdecmps.MONITOR_HRLY_VALUE AS O2D 
