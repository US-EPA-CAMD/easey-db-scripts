-- PROCEDURE: camdecmps.refresh_emission_view_co2appd()

DROP PROCEDURE IF EXISTS camdecmps.refresh_emission_view_co2appd();

CREATE OR REPLACE PROCEDURE camdecmps.refresh_emission_view_co2appd()
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
	TRUNCATE camdecmps.EMISSION_VIEW_CO2APPD RESTART IDENTITY;
	...
	LEFT OUTER JOIN camdecmps.HRLY_PARAM_FUEL_FLOW HPFF_CO2 
		ON ((HFF.HRLY_FUEL_FLOW_ID = HPFF_CO2.HRLY_FUEL_FLOW_ID) AND (HPFF_CO2.PARAMETER_CD = 'CO2'))		
	LEFT OUTER JOIN camdecmps.HRLY_PARAM_FUEL_FLOW HPFF_HI 
		ON ((HFF.HRLY_FUEL_FLOW_ID = HPFF_HI.HRLY_FUEL_FLOW_ID) AND (HPFF_HI.PARAMETER_CD = 'HI'))
	LEFT OUTER JOIN camdecmps.HRLY_PARAM_FUEL_FLOW HPFF_FC 	
	...
	LEFT OUTER JOIN camdecmps.MONITOR_FORMULA SUMMATION_MF
