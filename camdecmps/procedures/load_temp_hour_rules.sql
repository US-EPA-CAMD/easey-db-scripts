-- PROCEDURE: camdecmps.load_temp_hour_rules(character varying, numeric)

DROP PROCEDURE IF EXISTS camdecmps.load_temp_hour_rules(character varying, numeric);

CREATE OR REPLACE PROCEDURE camdecmps.load_temp_hour_rules(
...
BEGIN
  IF NOT EXISTS (
    ...
    INNER JOIN camdecmps.EMISSION_EVALUATION evl 
      ON evl.MON_PLAN_ID = ge.MON_PLAN_ID 
      AND evl.RPT_PERIOD_ID = ge.RPT_PERIOD_ID		
    INNER JOIN camdecmps.HRLY_OP_DATA hod 
      ON hod.HOUR_ID = ge.HOUR_ID 
    INNER JOIN camdecmpsaux.CHECK_LOG log
