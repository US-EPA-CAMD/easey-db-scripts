-- PROCEDURE: camdecmps.refresh_emission_view_co2calc(character varying, numeric)
...
BEGIN
	...
	LEFT OUTER JOIN camdecmps.MONITOR_HRLY_VALUE AS H2O_MHV 
		ON DHV.HOUR_ID = H2O_MHV.HOUR_ID
		AND DHV.MON_LOC_ID = H2O_MHV.MON_LOC_ID
		AND DHV.RPT_PERIOD_ID = H2O_MHV.RPT_PERIOD_ID
		AND H2O_MHV.PARAMETER_CD = 'H2O'
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE AS H2O_DHV 
		ON DHV.HOUR_ID = H2O_DHV.HOUR_ID
		AND DHV.MON_LOC_ID = H2O_DHV.MON_LOC_ID
		AND DHV.RPT_PERIOD_ID = H2O_DHV.RPT_PERIOD_ID
	...
	LEFT OUTER JOIN camdecmps.MONITOR_HRLY_VALUE AS DRY_MHV 
		ON DHV.HOUR_ID = DRY_MHV.HOUR_ID
		AND DHV.MON_LOC_ID = DRY_MHV.MON_LOC_ID
		AND DHV.RPT_PERIOD_ID = DRY_MHV.RPT_PERIOD_ID
		AND DRY_MHV.PARAMETER_CD = 'O2C'
		AND DRY_MHV.MOISTURE_BASIS = 'D'
	LEFT OUTER JOIN camdecmps.MONITOR_HRLY_VALUE AS WET_MHV 
		ON DHV.HOUR_ID = WET_MHV.HOUR_ID
		AND DHV.MON_LOC_ID = DRY_MHV.MON_LOC_ID
		AND DHV.RPT_PERIOD_ID = DRY_MHV.RPT_PERIOD_ID
