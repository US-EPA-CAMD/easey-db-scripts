-- PROCEDURE: camdecmps.refresh_emission_view_all(character varying, numeric)
...
BEGIN
	...
	LEFT JOIN camdecmps.DERIVED_HRLY_VALUE dhv_SO2
	...
	LEFT JOIN camdecmps.DERIVED_HRLY_VALUE dhv_NOXR
	...
	LEFT JOIN camdecmps.MONITOR_HRLY_VALUE AS FLOW_MHV
...
/*
  -- EXTREMELY INEFFICIENT WHEN LOTS OF DATA
	...
	LEFT OUTER JOIN camdecmps.MONITOR_FORMULA mf_HI
		ON dhv_HI.MON_FORM_ID = mf_HI.MON_FORM_ID
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE dhv_SO2
		ON ((dhv_SO2.HOUR_ID = hod.HOUR_ID) AND (dhv_SO2.PARAMETER_CD = 'SO2'))
	LEFT OUTER JOIN camdecmps.MONITOR_FORMULA mf_SO2
		ON dhv_SO2.MON_FORM_ID = mf_SO2.MON_FORM_ID
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE dhv_NOXR
		ON ((dhv_NOXR.HOUR_ID = hod.HOUR_ID) AND (dhv_NOXR.PARAMETER_CD = 'NOXR'))
	LEFT OUTER JOIN camdecmps.MONITOR_FORMULA mf_NOXR
		ON dhv_NOXR.MON_FORM_ID = mf_NOXR.MON_FORM_ID
	...
	LEFT OUTER JOIN camdecmps.MONITOR_FORMULA mf_NOX
		ON dhv_NOX.MON_FORM_ID = mf_NOX.MON_FORM_ID
	LEFT OUTER JOIN camdecmps.DERIVED_HRLY_VALUE dhv_CO2
		ON ((dhv_CO2.HOUR_ID = hod.HOUR_ID) AND (dhv_CO2.PARAMETER_CD = 'CO2'))
	LEFT OUTER JOIN camdecmps.MONITOR_FORMULA mf_CO2
		ON dhv_CO2.MON_FORM_ID = mf_CO2.MON_FORM_ID
	LEFT OUTER JOIN camdecmps.MONITOR_HRLY_VALUE AS FLOW_MHV
