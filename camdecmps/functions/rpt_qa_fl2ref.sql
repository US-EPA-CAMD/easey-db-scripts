DROP FUNCTION IF EXISTS camdecmps.rpt_qa_fl2ref(text) CASCADE;

CREATE OR REPLACE FUNCTION camdecmps.rpt_qa_fl2ref(
	testsumid text)
    RETURNS TABLE(
	"rataTestNumRef" text,
	"refRataEndDate" text,
...
FROM camdecmps.flow_to_load_reference ftl
