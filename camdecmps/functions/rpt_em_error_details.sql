-- FUNCTION: camdecmps.rpt_em_error_details(text, date, numeric)

DROP FUNCTION IF EXISTS camdecmps.rpt_em_error_details(text, date, numeric) CASCADE;

CREATE OR REPLACE FUNCTION camdecmps.rpt_em_error_details(
	vmonplanid text,
  vdate date,
  vhour numeric  
)
...
AS $BODY$
	SELECT
		...
		camdecmps.format_date_hour(cl.op_begin_date, cl.op_begin_hour, null) AS "beginPeriod",
		camdecmps.format_date_hour(cl.op_end_date, cl.op_end_hour, null) AS "endPeriod",
	...
	JOIN camdecmpsaux.check_session cs USING(chk_session_id)
