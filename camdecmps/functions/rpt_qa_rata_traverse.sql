DROP FUNCTION IF EXISTS camdecmps.rpt_qa_rata_traverse(text,text);

CREATE OR REPLACE FUNCTION camdecmps.rpt_qa_rata_traverse(
	testsumid text,
	oplevelcd text)
    RETURNS TABLE(
		"runNum" numeric,
...
SELECT
	...
	join camdecmps.flow_rata_run frr using(flow_rata_run_id)
	join camdecmps.rata_run rr using(rata_run_id)
