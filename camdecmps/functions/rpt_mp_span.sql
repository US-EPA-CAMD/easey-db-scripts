-- FUNCTION: camdecmps.rpt_mp_span(character varying)

DROP FUNCTION IF EXISTS camdecmps.rpt_mp_span(character varying) CASCADE;

CREATE OR REPLACE FUNCTION camdecmps.rpt_mp_span(
...
SELECT
		...
		camdecmps.format_date_hour(ms.begin_date, ms.begin_hour, null) AS "beginDateHour",
		camdecmps.format_date_hour(ms.end_date, ms.end_hour, null) AS "endDateHour"
	FROM camdecmps.monitor_span ms
