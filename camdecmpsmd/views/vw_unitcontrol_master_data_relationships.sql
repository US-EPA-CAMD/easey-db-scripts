...
CREATE OR REPLACE VIEW camdecmpsmd.vw_unitcontrol_master_data_relationships
 AS
 SELECT unit_control.parameter_code,
   ...
   FROM ( SELECT param.value1 AS parameter_code
     ...
     LEFT JOIN ( SELECT DISTINCT vw_cross_check_catalog_value.value1 AS parameter_code,
          ...
          WHERE vw_cross_check_catalog_value.cross_chk_catalog_name::text = 'Parameter Code to Control Code for Unit Control'::text) control_code ON unit_control.parameter_code = control_code.parameter_code;
