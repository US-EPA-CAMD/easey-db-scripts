-- PROCEDURE: camddmw.annual_unit_data_sync()

DROP PROCEDURE IF EXISTS camddmw.annual_unit_data_sync();

CREATE OR REPLACE PROCEDURE camddmw.annual_unit_data_sync(
	)
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
	DELETE FROM camddmw.annual_unit_data aud
	USING (
		SELECT unit_id, op_year
		FROM camddmw.annual_unit_data_log
	...
	INSERT INTO camddmw.annual_unit_data
	...
	FROM camddmw.annual_unit_data_log l
	JOIN (
	...
	UPDATE camddmw.annual_unit_data aud
