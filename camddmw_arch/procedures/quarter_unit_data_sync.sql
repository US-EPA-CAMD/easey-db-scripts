-- PROCEDURE: camddmw_arch.quarter_unit_data_a_sync()

-- DROP PROCEDURE camddmw_arch.quarter_unit_data_a_sync();

CREATE OR REPLACE PROCEDURE camddmw_arch.quarter_unit_data_a_sync()
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
	DELETE FROM camddmw_arch.quarter_unit_data_a qud
	USING (
		SELECT unit_id, op_year, op_quarter
		FROM camddmw_arch.quarter_unit_data_a_log
	...
	FROM camddmw_arch.quarter_unit_data_a_log l
	JOIN (
	...
	UPDATE camddmw_arch.quarter_unit_data_a qud
