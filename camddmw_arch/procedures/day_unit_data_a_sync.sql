-- PROCEDURE: camddmw_arch.day_unit_data_a_sync()

DROP PROCEDURE IF EXISTS camddmw_arch.day_unit_data_a_sync();

CREATE OR REPLACE PROCEDURE camddmw_arch.day_unit_data_a_sync()
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
	DELETE FROM camddmw_arch.day_unit_data_a dud
	USING (
		SELECT unit_id, op_date
		FROM camddmw_arch.day_unit_data_a_log
	...
	FROM camddmw_arch.day_unit_data_a_log l
	JOIN (
	...
	UPDATE camddmw_arch.day_unit_data_a dud
