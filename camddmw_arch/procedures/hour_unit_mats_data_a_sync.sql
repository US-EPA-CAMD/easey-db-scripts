-- PROCEDURE: camddmw_arch.hour_unit_mats_data_a_sync()

DROP PROCEDURE IF EXISTS camddmw_arch.hour_unit_mats_data_a_sync();

CREATE OR REPLACE PROCEDURE camddmw_arch.hour_unit_mats_data_a_sync(
	)
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
	DELETE FROM camddmw_arch.hour_unit_mats_data_a hud
	USING (
		SELECT unit_id, op_date, op_hour
		FROM camddmw_arch.hour_unit_mats_data_a_log
	...
	FROM camddmw_arch.hour_unit_mats_data_a_log l
	JOIN (
	...
	UPDATE camddmw_arch.hour_unit_mats_data_a hud
