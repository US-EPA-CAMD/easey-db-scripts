-- PROCEDURE: camddmw_arch.ozone_unit_data_a_sync()

DROP PROCEDURE IF EXISTS camddmw_arch.ozone_unit_data_a_sync();

CREATE OR REPLACE PROCEDURE camddmw_arch.ozone_unit_data_a_sync()
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
	DELETE FROM camddmw_arch.ozone_unit_data_a oud
	USING (
		SELECT unit_id, op_year
		FROM camddmw_arch.ozone_unit_data_a_log
	...
	FROM camddmw_arch.ozone_unit_data_a_log l
	JOIN (
	...
	UPDATE camddmw_arch.ozone_unit_data_a oud
