...
/****** Object:  UserDefinedFunction [SqlGenEm].[PgDailyEmissionDeletes]    Script Date: 2/8/2023 4:25:57 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER FUNCTION SqlGenEm.PgDailyEmissionDeletes
(	
    ...
    select  SqlGenEm.FormatSql
            (
                ...
                (
                    ...
                    ' where MON_LOC_ID = ''', sel.MON_LOC_ID, ''' and BEGIN_DATE = ''', format( sel.BEGIN_DATE, 'yyyy-MM-dd' ), ''';' 
                )
            ) as SQL_STATEMENT
      from  (
                select  -- ECMPSP
