...
/****** Object:  UserDefinedFunction [SqlGenEm].[PgWeeklyTestSummaryInserts]    Script Date: 2/8/2023 2:37:38 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER   FUNCTION SqlGenEm.PgWeeklyTestSummaryInserts
(	
            ...
            (
                ...
                (
                    ...
                    ' (',
                        ...
                        case when  tar.TEST_DATE                      is not null  then  concat( '''', format( tar.TEST_DATE, 'yyyy-MM-dd'), '''' ) else 'NULL' end, ', ',
                        case when  tar.TEST_HOUR                      is not null  then  cast( tar.TEST_HOUR as varchar ) else 'NULL' end, ', ',
                        case when  tar.TEST_MIN                       is not null  then  cast( tar.TEST_MIN as varchar ) else 'NULL' end, ', ',
