...
/****** Object:  UserDefinedFunction [SqlGenQa].[PgQceStatements]    Script Date: 11/23/2022 2:40:13 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE or ALTER FUNCTION SqlGenQa.PgQceStatements 
(	
	@OrisCode           integer,
...
BEGIN
    ...
    declare @vInformationTable SqlGenQa.Qce_Information_Table;
    ...
    select  lst.QA_CERT_EVENT_ID,
            lst.ORIS_CODE,
            lst.FACILITY_NAME,
            lst.LOCATION_NAME,
            lst.QA_CERT_EVENT_CD,
            lst.QA_CERT_EVENT_DATE,
            lst.QA_CERT_EVENT_HOUR,
            lst.SYSTEM_IDENTIFIER,
            lst.SYS_TYPE_CD,
            lst.COMPONENT_IDENTIFIER,
            lst.COMPONENT_TYPE_CD
      from  SqlGenQa.PgQceInformation( @OrisCode, @LocationNameFilter ) lst;
    ...
    select  SqlGenQa.FormatQceSql
            ( 
                lst.ORIS_CODE, lst.FACILITY_NAME, lst.LOCATION_NAME, lst.QA_CERT_EVENT_CD, lst.QA_CERT_EVENT_DATE, lst.QA_CERT_EVENT_HOUR, lst.SYSTEM_IDENTIFIER, lst.SYS_TYPE_CD, lst.COMPONENT_IDENTIFIER, lst.COMPONENT_TYPE_CD, 1,
                concat( 'delete from camdecmpswks.QA_CERT_EVENT where QA_CERT_EVENT_ID = ', '''', lst.QA_CERT_EVENT_ID, '''', ';' )
            ) as SQL_STATEMENT
    ...
    select  SqlGenQa.FormatQceSql
            ( 
                lst.ORIS_CODE, lst.FACILITY_NAME, lst.LOCATION_NAME, lst.QA_CERT_EVENT_CD, lst.QA_CERT_EVENT_DATE, lst.QA_CERT_EVENT_HOUR, lst.SYSTEM_IDENTIFIER, lst.SYS_TYPE_CD, lst.COMPONENT_IDENTIFIER, lst.COMPONENT_TYPE_CD, 2,
                concat
                (
                    'insert into ', 
