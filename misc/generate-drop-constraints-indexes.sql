...
BEGIN
	...
	LOOP
		...
		LOOP
			sqlStatement := sqlStatement || format('DROP INDEX IF EXISTS %s.%s;', curSchema, curIndex.indexname) || chr(10);
		END LOOP;
		...
		LOOP
			IF (index = conCount) THEN
				sqlStatement := sqlStatement || format('	DROP CONSTRAINT IF EXISTS %s;', curCon.conname) || chr(10);
			ELSE
				sqlStatement := sqlStatement || format('	DROP CONSTRAINT IF EXISTS %s,', curCon.conname) || chr(10);
			END IF;
			index := index + 1;
